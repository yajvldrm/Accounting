...
#Region "Variable Declaration"
    ...
    Private module_code As String = "ACC00501"
...
#Region "Other Functions/Subs"
    ...
    Private Sub enable_control()
        ...
        Dim dr As DataRow = main.Sys_Default_Det.Select("module_code = 'ACC00501'")(0)
...
#Region "Events"
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        report_jb_filter_new.journal_id = JournalID
        report_jb_filter_new.book_id = BookID
        report_jb_filter_new.transid = txt_trans_id.Text
        report_jb_filter_new.transno = txt_trans_no.Text
        report_jb_filter_new.ShowDialog()
    End Sub
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        For i = 0 To ds.Tables("jbooks").Rows.Count - 1
            ...
            Dim jbookstransid As String = IIf(dr("trans_id").ToString = "" Or dr("trans_id").ToString = "0", trans_id, dr("trans_id").ToString)
            ...
            Dim refjbooksid As String = IIf(dr("ref_jbooks_id").ToString = "" Or dr("ref_jbooks_id").ToString = "0", Nothing, dr("ref_jbooks_id").ToString)
            ...
            For i = 0 To ds.Tables("output_vat").Rows.Count - 1
                ...
                If dr("output_vat_id").ToString = "" Then
                    If dr("deleted").ToString = "False" Then
                        ...
                        qry = ATGenerateQuery("Insert", "trans_output_vat", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("output_vat_id").ToString, "outpout_vat_id")
                    End If
                Else
                    If dr("deleted").ToString = "True" Then
    ...
    Private Sub btnRemove_Click(sender As Object, e As EventArgs) Handles btnRemove.Click
        ...
        For Each dr As DataRow In dt.Rows
            If arr_id.Contains(ifNull(dr.Item("jbooks_id"), "").ToString()) Or arr_id.Contains(ifNull(dr.Item("jbooks_id_tmp"), "").ToString()) Then
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        End If
        computeTotal()
        Dim tmp_debit = ds.Tables("jbooks").Compute("sum(credit)", "deleted = 'False' AND noCompute = 'False'")
    ...
    Private Sub txtdnref_Leave(sender As Object, e As EventArgs) Handles txt_ref_jbooks_id.Leave
        If txt_ref_jbooks_id.Enabled Then
            ...
            dgjbooks.DataSource = ds.Tables("jbooks")
            computeTotal()
            Dim tmp_debit = ds.Tables("jbooks").Compute("sum(credit)", "deleted = 'False'")
            txtAmount.EditValue = If(tmp_debit.ToString = "", 0, tmp_debit)
            txtAmountBased.EditValue = txtAmount.EditValue * (txtExchangeRate.EditValue / txtBasedRate.EditValue)
            If CDec(txtAmountBased.EditValue) < 0 Then
                txt_amount_in_word.Text = "NEGATIVE " & amount_words(-1 * CDec(txtAmountBased.EditValue), main.DecimalPlaces)
    ...
    Private Sub btnParticular_Click(sender As Object, e As EventArgs) Handles btnParticular.Click
        ...
        trans_particular.TransactionName = "Journal Voucher"
...
#End Region
    ...
    Private Sub btnRestatement_Click(sender As Object, e As EventArgs) Handles btnRestatement.Click
        Dim general_id As String = ifNull(txt_general_id.Value, "").ToString
        Dim currency_id As String = ifNull(txt_currency_id.Value, "").ToString
        ...
        Dim book_id As String = BookID
        Dim trans_date As Date = New Date(txt_year.Text, txt_month.Value, txt_day.Text)
        If general_id = "" Then
            MsgBox("Setup Client first.")
            Exit Sub
        End If
        If currency_id = "" Then
            MsgBox("Setup Currency first.")
            Exit Sub
        End If
        Dim col As New List(Of KeyValuePair(Of String, String))
        col.Add(New KeyValuePair(Of String, String)("@dbName", _serverDBMain))
        col.Add(New KeyValuePair(Of String, String)("@book_id", book_id))
        col.Add(New KeyValuePair(Of String, String)("@trans_date", trans_date.ToString("yyyy-MM-dd")))
        col.Add(New KeyValuePair(Of String, String)("@currency_id", currency_id))
        col.Add(New KeyValuePair(Of String, String)("@exchange_rate", exchange_rate))
        col.Add(New KeyValuePair(Of String, String)("@based_rate", based_rate))
        Dim dt As DataTable = ExecuteWithParameter("get_entry_for_restatement", ConnectionStringFS, CommandType.StoredProcedure, col)
        If dt.Rows.Count > 0 Then
            For Each dr As DataRow In dt.Rows
                ds.Tables("jbooks").ImportRow(dr)
            Next
            computeTotal()
            loadDTToDG(dgjbooks, ds.Tables("jbooks"))
        Else
            MsgBox("No account available to restate")
        End If
    End Sub

    Private Sub transactionjb_Load(sender As Object, e As EventArgs) Handles MyBase.Load
