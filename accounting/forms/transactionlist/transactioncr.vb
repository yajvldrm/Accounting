...
#Region "Variable Declaration"
    ...
    Private module_code As String = "ACC00201"
    ...
    Public Shared offsetList_ As DataTable
#End Region
#Region "Property"
    Public Property entryValidated As Boolean = False
    Public Property CreateORFromAR As Boolean = False
...
#Region "Other Functions/Subs"
    Public Sub ReverseAR(ByRef drTrans As DataRow, ByRef dtJBooks As DataTable, ByRef dtChecks As DataTable)
    ...
    Friend Sub loadLastTransaction(Optional pageNumber As Integer = 0)
        ...
        ",IFNULL(b.trans_cr_id,'') AS trans_cr_id,IFNULL(b.si_no,'') AS si_no " &
        ",IFNULL(_lock.isLock,0) AS isLock  " &
        "FROM trans_header a " &
        "LEFT JOIN selectTransactionLocking _lock ON _lock.trans_id = a.trans_id " &
        "LEFT JOIN trans_header_cr b ON a.trans_id = b.trans_id " &
...
#Region "Events"
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        report_cr_filter_new.journal_id = JournalID
        report_cr_filter_new.book_id = BookID
        report_cr_filter_new.transid = txt_trans_id.Text
        report_cr_filter_new.trans_no = txt_trans_no.Text
        report_cr_filter_new.ShowDialog()
    End Sub
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        qry = ""
        fields = New Dictionary(Of String, String)
        fields.Add("trans_cr_id", txt_trans_cr_id.Text)
        fields.Add("trans_id", trans_id)
        fields.Add("si_no", txt_si_no.Text)
        If txt_trans_cr_id.Text = "" Then
            'qry = genInsert("trans_header_cr", fields)
            qry = ATGenerateQuery("Insert", "trans_header_cr", fields, Environment.MachineName, Environment.UserName, main.log_id, txt_trans_cr_id.Text, "trans_cr_id")
        Else
            'qry = genUpdate("trans_header_cr", fields)
            qry = ATGenerateQuery("Update", "trans_header_cr", fields, Environment.MachineName, Environment.UserName, main.log_id, txt_trans_cr_id.Text, "trans_cr_id")
        End If
        dt = Query(qry)
        ...
        For i = 0 To ds.Tables("jbooks").Rows.Count - 1
            ...
            If CDec(ifNull(dr("debit"), 0)) = 0 And CDec(ifNull(dr("debit_based"), 0)) = 0 And CDec(ifNull(dr("credit"), 0)) = 0 And CDec(ifNull(dr("credit_based"), 0)) = 0 Then
                dr("deleted") = True
            End If
        ...
        'Next
        If ds.Tables("project_management_offset").Rows.Count > 0 Then
            For i As Integer = 0 To ds.Tables("project_management_offset").Rows.Count - 1
                qry = ""
                Dim dr As DataRow = ds.Tables("project_management_offset").Rows(i)
                fields = New Dictionary(Of String, String)
                fields.Add("offset_project_management_id", dr("offset_project_management_id").ToString)
                ...
                Else
                    ...
                    'fields.Add("jbooks_id", If(dr("jbooks_id").ToString = "", tmp_dt.Rows(0).Item("id").ToString, dr("jbooks_id").ToString))
                End If
                fields.Add("trans_project_management_id", dr("trans_project_management_id").ToString)
                fields.Add("amount_collected", dr("amount_collected").ToString)
                fields.Add("amount_collected_based", dr("amount_collected_based").ToString)
                fields.Add("currency_id", dr("currency_id").ToString)
                fields.Add("exchange_rate", dr("exchange_rate").ToString)
                fields.Add("based_rate", dr("based_rate").ToString)

                If dr("offset_project_management_id").ToString = "" Then
                    If dr("deleted").ToString = "False" Then
                        'qry = genInsert("trans_ewt", fields)
                        qry = ATGenerateQuery("Insert", "trans_project_management_offset", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("offset_project_management_id").ToString, "offset_project_management_id")
                    End If
                Else
                    If dr("deleted").ToString = "True" Then
                        'qry = String.Format("DELETE FROM trans_ewt WHERE ewt_id = {0}", dr("ewt_id").ToString)
                        qry = ATGenerateQuery("Remove", "trans_project_management_offset", Nothing, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("offset_project_management_id").ToString & "|", "offset_project_management_id")
                    Else
                        'qry = genUpdate("trans_ewt", fields)
                        qry = ATGenerateQuery("Update", "trans_project_management_offset", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("offset_project_management_id").ToString, "offset_project_management_id")
                    End If
                End If
                If qry <> "" Then
                    dt = Query(qry)
        ...
        If ds.Tables("checks").Rows.Count > 0 Then
            For i = 0 To ds.Tables("checks").Rows.Count - 1
                ...
                fields.Add("bank_code", dr("bank_code").ToString)
                fields.Add("bank_name", dr("bank_name").ToString)
        ...
        If selectedRows.Count > 0 Then
            ...
            If selectedRow.Cells("offset").Value > 0 Then
                If "1456789".Contains(selectedRow.Cells("validation").Value.ToString) Then
                    ...
                    btnOK.Focus()
                    entryValidated = False
                    EntryValidate.NewValidateEntry(Me, dgjbooks, ds, True, BookCode, JournalCode, BookID)
                    btnOK.PerformClick()
                    Exit Sub
                End If
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        If "456789".Contains(v.ToString) Then
            If Not entryValidated Then
                ...
                flag = True
            End If
            'If "1268".Contains(v.ToString) Then
        ...
        If v.ToString = "1" Then
            If ds.Tables("jbooks").Select("validation = '1'").Count > 0 Then
                Dim tmp_debit = ds.Tables("jbooks").Compute("sum(debit)", "validation = '1' AND deleted = 'False' AND noCompute = 'False'")
        ...
        If "457".Contains(v.ToString) Then
            ...
            End If
        End If
        If "1689".Contains(v.ToString) Then
    ...
    Private Sub txtdebit_Leave(sender As Object, e As EventArgs) Handles txtDebit.Leave
        If txtDebit.Enabled Then
            ...
            Else
                ...
                txtCredit.Focus()
            End If
            If "1456789".IndexOf(v.ToString) >= 0 And (txtDebit.EditValue <> 0 Or txtCredit.EditValue <> 0) Then
    ...
    Private Sub txtdnref_Leave(sender As Object, e As EventArgs) Handles txt_ref_jbooks_id.Leave
        If txt_ref_jbooks_id.Enabled Then
            ...
            If v.ToString = "1" Then
                If ds.Tables("jbooks").Select("validation = '1'").Count > 0 Then
                    Dim tmp_debit = ds.Tables("jbooks").Compute("sum(debit)", "validation = '1' AND deleted = 'False'")
                    txtAmount.EditValue = If(tmp_debit.ToString = "", 0, tmp_debit)
                    txtAmountBased.EditValue = txtAmount.EditValue * (txtExchangeRate.EditValue / txtBasedRate.EditValue)
                    If CDec(txtAmountBased.EditValue) < 0 Then
                        txt_amount_in_word.Text = "NEGATIVE " & amount_words(-1 * CDec(txtAmountBased.EditValue), main.DecimalPlaces)
    ...
    Private Sub btnParticular_Click(sender As Object, e As EventArgs) Handles btnParticular.Click
        ...
        trans_particular.TransactionName = "Official Receipt"
...
#End Region
    ...
    Private Sub transactioncr_Load(sender As Object, e As EventArgs) Handles MyBase.Load
