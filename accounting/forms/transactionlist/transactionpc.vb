...
#Region "Other Functions/Subs"
    ...
    Friend Sub loadLastTransaction(Optional pageNumber As Integer = 0)
        ...
        ",IFNULL(b.trans_pc_id,'') AS trans_pc_id,b.reflenish_date,IFNULL(b.po_no,'') AS po_no,IFNULL(b.dr_no,'') AS dr_no " &
        ",IFNULL(b.si_no,'') AS si_no,IFNULL(b.terms_id,'') AS terms_id,IFNULL(b.nodays,0) AS nodays,b.due_date " &
        ",IFNULL(e.terms_name,'') AS terms_name " &
        ",IFNULL(_lock.isLock,0) AS isLock  " &
        "FROM trans_header a " &
        "LEFT JOIN selectTransactionLocking _lock ON _lock.trans_id = a.trans_id " &
        "LEFT JOIN trans_header_pc b ON a.trans_id = b.trans_id " &
        "LEFT JOIN {0}.lib_erp_general c ON a.general_id = c.general_id " &
        "LEFT JOIN {0}.lib_erp_currency d ON a.currency_id = d.currency_id " &
        "LEFT JOIN {0}.lib_erp_terms e ON e.terms_id = b.terms_id " &
        "LEFT JOIN ( " &
          "SELECT * " &
    ...
    Friend Sub refresh_record()
        ...
        "LEFT JOIN {0}.lib_erp_general c ON a.general_id = c.general_id " &
        "LEFT JOIN {0}.lib_erp_currency d ON a.currency_id = d.currency_id " &
        "LEFT JOIN {0}.lib_erp_terms e ON e.terms_id = a.terms_id " &
        "LEFT JOIN ( " &
          "SELECT * " &
          "FROM trans_header_status " &
          "GROUP BY ref_id " &
...
#Region "Events"
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        report_pc_filter_new.journal_id = JournalID
        report_pc_filter_new.book_id = BookID
        report_pc_filter_new.transid = txt_trans_id.Text
        report_pc_filter_new.transno = txt_trans_no.Text
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        qry = ""
        fields = New Dictionary(Of String, String)
        fields.Add("trans_pc_id", txt_trans_pc_id.Text)
        fields.Add("trans_id", trans_id)
        fields.Add("reflenish_date", txt_reflenish_date.Value.ToString("yyyy-MM-dd"))
        ...
        fields.Add("due_date", txt_due_date.Value.ToString("yyyy-MM-dd"))
        If txt_trans_pc_id.Text = "" Then
            'qry = genInsert("trans_header_pc", fields)
            qry = ATGenerateQuery("Insert", "trans_header_pc", fields, Environment.MachineName, Environment.UserName, main.log_id, txt_trans_pc_id.Text, "trans_pc_id")
        Else
            'qry = genUpdate("trans_header_pc", fields)
            qry = ATGenerateQuery("Update", "trans_header_pc", fields, Environment.MachineName, Environment.UserName, main.log_id, txt_trans_pc_id.Text, "trans_pc_id")
        End If
        dt = Query(qry)
    ...
    Private Sub btnParticular_Click(sender As Object, e As EventArgs) Handles btnParticular.Click
        ...
        trans_particular.TransactionName = "Petty Cash Voucher"
...
#End Region
    ...
    Private Sub transactionpc_Load(sender As Object, e As EventArgs) Handles MyBase.Load
