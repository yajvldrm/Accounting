...
#Region "Variable Declaration"
    ...
    Public public_table As DataTable
...
#Region "Property"
    ...
    Public Shared Property Deleted_datatable As New List(Of Integer)
    Public apVar_trans_id As String
...
#Region "Other Functions/Subs"
    Friend Sub initializeDecimalPlaces(ByVal strNumberFormat As String)
        ...
        dgjbooks.Columns("debit_based").DefaultCellStyle.Format = "n" & main.DecimalPlaces
        dgjbooks.Columns("credit_based").DefaultCellStyle.Format = "n" & main.DecimalPlaces
    End Sub
    Friend Sub loadLastTransaction(Optional pageNumber As Integer = 0)
        txt_book_id.Value = BookID
        txt_book_id.Text = BookID
        ...
        ",IFNULL(b.trans_ap_id,'') AS trans_ap_id,IFNULL(b.si_no,'') AS si_no,IFNULL(b.rr_no,'') AS rr_no,IFNULL(b.po_no,'') AS po_no " &
        ...
        "LEFT JOIN trans_header_ap b ON a.trans_id = b.trans_id " &
    ...
    Public Sub disable_control(Optional creator As Boolean = False, Optional dr As DataRow = Nothing)
        ...
        btnCreateCV.Visible = main.dicAction.Item("ACC00401").Item("m_add")
        btnSubmit.Visible = False
        btnApprove.Visible = False
        btnCorrection.Visible = False
        btnComment.Visible = False
    ...
    Friend Sub refresh_record()
        ...
        ",c.general_code,c.general_name " &
        ",d.currency_code,d.currency_name " &
        ",IFNULL(b.trans_ap_id,'') AS trans_ib_id,IFNULL(b.si_no,'') AS si_no,IFNULL(b.rr_no,'') AS dr_no,IFNULL(b.po_no,'') AS po_no " &
        ",IFNULL(b.terms_id,'') AS terms_id,IFNULL(b.nodays,0) AS nodays,e.terms_name,b.due_date " &
        "FROM trans_header a " &
        "LEFT JOIN trans_header_ap b ON a.trans_id = b.trans_id " &
        "LEFT JOIN {0}.lib_erp_general c ON a.general_id = c.general_id " &
...
#Region "Events"
    ...
    Private Sub CheckEditable()
        For Each dr As DataRow In ds.Tables("trans_jbooks").Rows

        Next
    End Sub
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        'report_ap_filter.journal_id = JournalID
        'report_ap_filter.book_id = BookID
        'report_ap_filter.transid = txt_trans_id.Text
        'report_ap_filter.trans_no = txt_trans_no.Text
        'report_ap_filter.ShowDialog()

        'emon - 2018-05-20 13:19
        report_ap_filter_new.journal_id = JournalID
        report_ap_filter_new.book_id = BookID
        report_ap_filter_new.transid = txt_trans_id.Text
        report_ap_filter_new.trans_no = txt_trans_no.Text
        report_ap_filter_new.ShowDialog()
    End Sub
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
            ...
            If dt_test_rans_no.Rows.Count > 0 Then
                ...
                If dr("auto_number") = 1 Then
                    ...
                    If res = MsgBoxResult.Yes Then
                        ...
                        apVar_trans_id = strTransNo
                        'Dim drss = ds.Tables("trans_sub_contract").Select(String.Format("trans_no<>{0}", txt_trans_no.Text))
                        'Dim drow As DataRow = drss(0)
                        'drow.Item("trans_no") = strTransNo
        ...
        qry = ""
        fields = New Dictionary(Of String, String)
        fields.Add("trans_ap_id", txt_trans_ap_id.Text)
        fields.Add("trans_id", trans_id)
        ...
        If txt_trans_ap_id.Text = "" Then
            'qry = genInsert("trans_header_ap", fields)
            qry = ATGenerateQuery("Insert", "trans_header_ap", fields, Environment.MachineName, Environment.UserName, main.log_id, txt_trans_ap_id.Text, "trans_ap_id")
        Else
            'qry = genUpdate("trans_header_ap", fields)
            qry = ATGenerateQuery("Update", "trans_header_ap", fields, Environment.MachineName, Environment.UserName, main.log_id, txt_trans_ap_id.Text, "trans_ap_id")
        End If
        dt = Query(qry)
        ...
        End If
        ''''SUBCONTRACT
        Dim sql_subCon_deleted As String = ""
        'For i As Integer = 0 To Deleted_datatable.Count - 1
        '    sql_subCon_deleted &= ATGenerateQuery("Remove", "trans_sub_contract_det_particulars", Nothing, Environment.MachineName, Environment.UserName, main.log_id, Deleted_datatable(i) & "|", "id")
        'Next
        'If sql_subCon_deleted <> "" Then
        '    Query(sql_subCon_deleted)
        'End If
        'For Each dr As DataRow In public_table.Rows
        '    ds.Tables("trans_sub_contract").ImportRow(dr)
        'Next
        Dim sql_subCon As String = ""
        For Each dr As DataRow In ds.Tables("trans_sub_contract").Rows
            fields = New Dictionary(Of String, String)
            Dim trans_sub_contract_id As String = dr.Item("trans_sub_contract_id")
            Dim sub_contract_det_id As String = dr.Item("sub_contract_det_id")
            Dim billed_amount As String = dr.Item("billed_amount")
            Dim billed_amount_based As String = dr.Item("billed_amount_based")
            Dim currency_id As String = dr.Item("currency_id")
            Dim exchange_rate As String = dr.Item("exchange_rate")
            Dim based_rate As String = dr.Item("based_rate")
            Dim deleted As Boolean = CBool(ifNull(dr.Item("deleted"), 0))

            fields.Add("trans_sub_contract_id", trans_sub_contract_id)
            fields.Add("trans_id", pkID)
            fields.Add("sub_contract_det_id", sub_contract_det_id)
            fields.Add("billed_amount", billed_amount)
            fields.Add("billed_amount_based", billed_amount_based)
            fields.Add("currency_id", currency_id)
            fields.Add("exchange_rate", exchange_rate)
            fields.Add("based_rate", based_rate)

            If ifNull(fields.Item("trans_sub_contract_id"), "").ToString = "" Then
                If Not deleted Then
                    qry = ATGenerateQuery("Insert", "trans_header_sub_contract", fields, Environment.MachineName, Environment.UserName, main.log_id, trans_sub_contract_id, "trans_sub_contract_id")
                End If
            Else
                If deleted Then
                    qry = ATGenerateQuery("Remove", "trans_header_sub_contract", Nothing, Environment.MachineName, Environment.UserName, main.log_id, trans_sub_contract_id & "|", "trans_sub_contract_id")
                Else
                    qry = ATGenerateQuery("Update", "trans_header_sub_contract", fields, Environment.MachineName, Environment.UserName, main.log_id, trans_sub_contract_id, "trans_sub_contract_id")
                End If
            End If
            If qry <> "" Then
                Query(qry)
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        If v.ToString = "3" Then
            If ds.Tables("jbooks").Select("validation = '3'").Count > 0 Then
                Dim tmp_debit = ds.Tables("jbooks").Compute("sum(credit)", "validation = '3' AND deleted = 'False' AND noCompute = 'False'")
    ...
    Private Sub txtdnref_Leave(sender As Object, e As EventArgs) Handles txt_ref_jbooks_id.Leave
        If txt_ref_jbooks_id.Enabled Then
            ...
            If v.ToString = "3" Then
                If ds.Tables("jbooks").Select("validation = '3'").Count > 0 Then
                    Dim tmp_debit = ds.Tables("jbooks").Compute("sum(credit)", "validation = '3' AND deleted = 'False'")
                    txtAmount.EditValue = If(tmp_debit.ToString = "", 0, tmp_debit)
                    txtAmountBased.EditValue = txtAmount.EditValue * (txtExchangeRate.EditValue / txtBasedRate.EditValue)
                    If CDec(txtAmountBased.EditValue) < 0 Then
                        txt_amount_in_word.Text = "NEGATIVE " & amount_words(-1 * CDec(txtAmountBased.EditValue), main.DecimalPlaces)
    ...
    Private Sub btnParticular_Click(sender As Object, e As EventArgs) Handles btnParticular.Click
        ...
        trans_particular.TransactionName = "Accounts Payable Voucher"
...
#End Region
    ...
    Private Sub btnCreateCV_Click(sender As Object, e As EventArgs) Handles btnCreateCV.Click
        Dim trans_id As String = txt_trans_id.Text
        Dim trans_no As String = txt_trans_no.Text
        Dim general_id As String = txt_general_id.Value
        Dim general_code As String = txt_general_id.Text
        Dim general_name As String = txt_general_name.Text
        Dim currency_id As String = txt_currency_id.Value
        Dim currency_code As String = txt_currency_id.Text
        Dim currency_name As String = txt_currency_name.Text
        Dim exchange_rate As Decimal = txtExchangeRate.EditValue
        Dim based_rate As Decimal = txtBasedRate.EditValue
        Dim account_id As String = ""
        Dim account_code As String = ""
        Dim account_name As String = ""
        Dim trans_date As Date = New Date(txt_year.Text, months(txt_month.Text), txt_day.Text)
        Dim totalAmount As Decimal = 0
        SetupAccountBankForm.trans_no = trans_no
        SetupAccountBankForm.txt_trans_no.Enabled = True
        SetupAccountBankForm.txt_bank_id.Enabled = True
        SetupAccountBankForm.txt_trans_date.Value = trans_date
        SetupAccountBankForm.ShowDialog()
        If SetupAccountBankForm.DialogResult = DialogResult.OK Then
            trans_no = SetupAccountBankForm.trans_no
            account_id = SetupAccountBankForm.account_id
            account_code = SetupAccountBankForm.account_code
            account_name = SetupAccountBankForm.account_name
            If account_id = "" Then
                MsgBox("No CIB Found")
            Else
                ...
                Dim dtJBooks As DataTable = ExecuteWithParameter("get_trans_apcv_det", ConnectionStringMain, CommandType.StoredProcedure, col)
                ...
                If dtTrans.Rows.Count > 0 And dtJBooks.Rows.Count > 0 Then
                    ...
                    cashcv.trans_date = SetupAccountBankForm.txt_trans_date.Value
                    cashcv.defaultDate = SetupAccountBankForm.txt_trans_date.Value
                    ...
                    If cashcv.ShowDialog() = DialogResult.OK Then
                        ...
                        transactioncv.BookID = BookID
                        transactioncv.BookCode = BookCode
    ...
    Private Sub btnSubCon_Click(sender As Object, e As EventArgs) Handles btnSubCon.Click
        ...
        Dim subCon As New subContract_Detailedparticular
        'subCon.dgView.DataSource = subCon.ds.Tables("trans_sub_contract_det")
        'subContract_Detailedparticular_det.subConTrans_ID = ifNull(txt_general_id.Value, 0)
        subCon.ds.Tables("trans_sub_contract_det").Clear()
        For Each dr As DataRow In ds.Tables("trans_sub_contract").Rows
            subCon.ds.Tables("trans_sub_contract_det").ImportRow(dr)
        Next

        subCon.ShowDialog()
        If subCon.DialogResult = DialogResult.OK Then
            ds.Tables("trans_sub_contract").Clear()
            Dim dt As DataTable = subCon.ds.Tables("trans_sub_contract_det")
            For Each dr As DataRow In dt.Rows
                ds.Tables("trans_sub_contract").ImportRow(dr)
            Next
        End If
        'subCon.dsssss.Tables("trans_sub_contract_det").Clear()
    End Sub
    Public Sub subconloader()
        'Try
