...
#Region "Variable Declaration"
    ...
    Public trans_aydi As String
#End Region
#Region "Property"
    Public Property entryValidated As Boolean = False
    Public Property CreateCVFromAP As Boolean = False
...
#Region "Other Functions/Subs"
    ...
    Friend Sub loadLastTransaction(Optional pageNumber As Integer = 0)
        ...
        ' GROUP BY c.trans_id
        ') AutoEntry ON AutoEntry.trans_id = a.trans_id " &
        ...
        ",IFNULL(b.trans_cv_id,'') AS trans_cv_id,IFNULL(b.si_no,'') AS si_no,IFNULL(b.rr_no,'') AS rr_no " &
        ",IFNULL(_lock.isLock,0) AS isLock  " &
        "FROM trans_header a " &
        "LEFT JOIN selectTransactionLocking _lock ON _lock.trans_id = a.trans_id " &
        "LEFT JOIN trans_header_cv b ON a.trans_id = b.trans_id " &
        ...
        "LEFT JOIN (
            ...
            GROUP BY a.trans_id
        ) checks ON checks.trans_id = a.trans_id " &
        ...
        End If
    End Sub
    Public Sub ReverseAP(ByRef drTrans As DataRow, ByRef dtJBooks As DataTable, ByRef dtChecks As DataTable, ByRef trans_no As String)
        If Not isInitialize Then
            init()
        End If
        clear_control()
        NewControls.bindControl(hdr, drTrans, True)
        currentDr = drTrans
        'disable_control(drTrans.Item("user_id").ToString = main.user_id, myDataTable.Rows(LastIndex))
        enable_control()
        txt_trans_no.Enabled = True
        txt_trans_no.Text = If(ifNull(drTrans.Item("trans_no"), "") = "", trans_no, drTrans.Item("trans_no"))
        'NewControls.refreshRecordDet(dgjbooks, ds, txt_trans_id)
        ds.Tables("jbooks").Clear()
        ds.Tables("checks").Clear()
        For Each dr As DataRow In dtJBooks.Rows
            ds.Tables("jbooks").ImportRow(dr)
        Next
        For Each dr As DataRow In dtChecks.Rows
            ds.Tables("checks").ImportRow(dr)
        Next
        If CDec(txtAmountBased.EditValue) < 0 Then
            txt_amount_in_word.Text = "NEGATIVE " & amount_words(-1 * CDec(txtAmountBased.EditValue), main.DecimalPlaces)
        ElseIf CDec(txtAmountBased.EditValue) > 0 Then
    ...
    Public Sub clear_control()
        ...
        Next
        'ds.Clear()
        lbldebittotal.Text = Format(0, txtDebit.Properties.DisplayFormat.FormatString)
    ...
    Private Sub enable_control()
        ...
        btnGetAp.Visible = main.dicAction.Item(module_code).Item("m_add")
...
#Region "Events"
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        dgjbooks.DataSource = ds.Tables("jbooks")
        lblStatus.Text = ""
        report_cv_erc_filter.btnInsert.Enabled = True
        report_cv_erc_filter.btnUpdate.Enabled = True
        report_cv_erc_filter.btnRemove.Enabled = True
    End Sub
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        report_cv_filter_new.journal_id = JournalID
        report_cv_filter_new.book_id = BookID
        report_cv_filter_new.transid = txt_trans_id.Text
        report_cv_filter_new.transno = txt_trans_no.Text
        report_cv_filter_new.ShowDialog()
    End Sub
    ...
    Private Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        ...
        TransactionListing.transactionName = "Purchase No"
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If txt_trans_no.Text = "" Then
            MsgBox("Cannot save transactions. Transaction No. is required.", MsgBoxStyle.OkOnly + MsgBoxStyle.Critical, "Invalid Record")
            btnSave.Enabled = True
            Exit Sub
        Else
        ...
        For Each dr As DataRow In ds.Tables("particular").Rows
            ...
            Else
                If dr.Item("deleted") Then
                    ...
                    sql_particular &= ATGenerateQuery("Remove", "trans_header_particular", Nothing, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("trans_particular_id").ToString & "|", "trans_particular_id")
                Else
                    'sql_particular &= genUpdate("trans_header_particular", fields) & ";"
                    sql_particular &= ATGenerateQuery("Update", "trans_header_particular", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("trans_particular_id").ToString, "trans_particular_id")
                End If
            End If
        Next
        ...
        For Each dr As DataRow In ds.Tables("detailed_particular").Rows
            fields = New Dictionary(Of String, String)
            ...
            If fields.Item("trans_detailed_particular_id").ToString.Contains("tmp_") Then
                If Not dr.Item("deleted") Then
                    sql_detailed_particular &= ATGenerateQuery("Insert", "trans_header_detailed_particular", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("trans_detailed_particular_id").ToString, "trans_detailed_particular_id")
                End If
            Else
                If dr.Item("deleted") Then
                    sql_detailed_particular &= ATGenerateQuery("Remove", "trans_header_detailed_particular", Nothing, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("trans_detailed_particular_id").ToString & "|", "trans_detailed_particular_id")
                Else
                    sql_detailed_particular &= ATGenerateQuery("Update", "trans_header_detailed_particular", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("trans_detailed_particular_id").ToString, "trans_detailed_particular_id")
                End If
            End If
        Next
        If sql_detailed_particular <> "" Then
            Query(sql_detailed_particular)
        End If
        ' trans_mfo_pap

        Dim sql_mfo_pap As String = ""
        For Each dr As DataRow In report_cv_erc_filter.ds.Tables("trans_mfo_pap").Rows
            fields = New Dictionary(Of String, String)
            fields.Add("mfo_pap_id", dr.Item("mfo_pap_id").ToString)
            fields.Add("trans_id", trans_id)
            fields.Add("mfo_pap_name", dr.Item("mfo_pap_name").ToString)
            fields.Add("mfo_pap_code", dr.Item("mfo_pap_code").ToString)
            fields.Add("mfo_responsibility_center", dr.Item("mfo_responsibility_center").ToString)

            If fields.Item("mfo_pap_id").ToString.Contains("tmp_") Then
                If Not dr.Item("deleted") Then
                    sql_mfo_pap &= ATGenerateQuery("Insert", "trans_mfo_pap", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("mfo_pap_id").ToString, "mfo_pap_id")
                End If
            Else
                If dr.Item("deleted") Then
                    sql_mfo_pap &= ATGenerateQuery("Remove", "trans_mfo_pap", Nothing, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("mfo_pap_id").ToString & "|", "mfo_pap_id")
                Else
                    sql_mfo_pap &= ATGenerateQuery("Update", "trans_mfo_pap", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("mfo_pap_id").ToString, "mfo_pap_id")
                End If
            End If
        Next
        If sql_mfo_pap <> "" Then
        ...
        For Each dr As DataRow In report_cv_erc_filter.ds.Tables("trans_additional").Rows
            fields = New Dictionary(Of String, String)
            fields.Add("trans_additional_id", dr.Item("trans_additional_id").ToString)
            fields.Add("trans_id", trans_id)
            fields.Add("ors_burs_no", report_cv_erc_filter.txt_ors_burs.Text.ToString)
            fields.Add("fund_cluster", report_cv_erc_filter.txt_fund_code.Text.ToString)
            fields.Add("mode_of_payment", report_cv_erc_filter.txt_mode_of_payment.Text.ToString)

            If fields.Item("trans_additional_id").ToString.Contains("tmp_") Then
                If Not dr.Item("deleted") Then
                    sql_additional &= ATGenerateQuery("Insert", "trans_additional", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("trans_additional_id").ToString, "trans_additional_id")
                End If
            Else
                If dr.Item("deleted") Then
                    sql_additional &= ATGenerateQuery("Remove", "trans_additional", Nothing, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("trans_additional_id").ToString & "|", "trans_additional_id")
                Else
                    sql_additional &= ATGenerateQuery("Update", "trans_additional", fields, Environment.MachineName, Environment.UserName, main.log_id, dr.Item("trans_additional_id").ToString, "trans_additional_id")
                End If
            End If
        Next
        If sql_additional <> "" Then
            Query(sql_additional)
        End If
        'ib
        qry = ""
        fields = New Dictionary(Of String, String)
        fields.Add("trans_cv_id", txt_trans_cv_id.Text)
        fields.Add("trans_id", trans_id)
        fields.Add("si_no", txt_si_no.Text)
        fields.Add("rr_no", txt_rr_no.Text)
        If txt_trans_cv_id.Text = "" Then
            'qry = genInsert("trans_header_cv", fields)
            qry = ATGenerateQuery("Insert", "trans_header_cv", fields, Environment.MachineName, Environment.UserName, main.log_id, txt_trans_cv_id.Text, "trans_cv_id")
        Else
            'qry = genUpdate("trans_header_cv", fields)
            qry = ATGenerateQuery("Update", "trans_header_cv", fields, Environment.MachineName, Environment.UserName, main.log_id, txt_trans_cv_id.Text, "trans_cv_id")
        End If
        dt = Query(qry)
        ...
        For i = 0 To ds.Tables("jbooks").Rows.Count - 1
            ...
            If qry <> "" Then
                ...
                If lastID.Rows.Count > 0 Then
                    If jbooks_ids.ContainsKey(dr("jbooks_id_tmp").ToString) Then
                        jbooks_ids.Item(dr("jbooks_id_tmp").ToString) = lastID.Rows(0).Item("return_id").ToString
                    Else
                        jbooks_ids.Add(dr("jbooks_id_tmp").ToString, lastID.Rows(0).Item("return_id").ToString)
                    End If
                Else
                    If jbooks_ids.ContainsKey(dr("jbooks_id_tmp").ToString) Then
                        jbooks_ids.Item(dr("jbooks_id_tmp").ToString) = dr.Item("jbooks_id").ToString
                    Else
            ...
            Else
                If jbooks_ids.ContainsKey(dr("jbooks_id_tmp").ToString) Then
                    jbooks_ids.Item(dr("jbooks_id_tmp").ToString) = ""
                Else
        ...
        For Each dr As DataRow In ds.Tables("trans_sub_contract_offset").Rows
            qry = ""
            fields = New Dictionary(Of String, String)
            fields.Add("offset_sub_contract_id", ifNull(dr("offset_sub_contract_id"), 0))
            Dim jid = dr("jbooks_id")
            If jbooks_ids.ContainsKey(jid) Then
                fields.Add("jbooks_id", jbooks_ids(jid))
            Else
                For j As Integer = 0 To jbooks_ids.Count - 1
                    Dim j_id = jbooks_ids.Keys(j)
                    If j_id.ToString.Length < jid.ToString.Length Then
                        If jid.Substring(0, j_id.ToString.Length) = j_id Then
                            fields.Add("jbooks_id", jbooks_ids(j_id))
                        End If
                    Else
                        If j_id.Substring(0, jid.ToString.Length) = jid Then
                            fields.Add("jbooks_id", jbooks_ids(j_id))
                        End If
                    End If
                Next
                'fields.Add("jbooks_id", If(dr("jbooks_id").ToString = "", tmp_dt.Rows(0).Item("id").ToString, dr("jbooks_id").ToString))
            End If
            fields.Add("trans_sub_contract_id", dr("trans_sub_contract_id"))
            fields.Add("amount_collected", dr("amount_collected"))
            fields.Add("amount_collected_based", dr("amount_collected_based"))
            fields.Add("currency_id", dr("currency_id"))
            fields.Add("exchange_rate", dr("exchange_rate"))
            fields.Add("based_rate", dr("based_rate"))
            If ifNull(fields("offset_sub_contract_id"), "").ToString = 0 Or ifNull(fields("offset_sub_contract_id"), "").ToString.Contains("tmp_") Then
                fields("offset_sub_contract_id") = ""
                If Not CBool(ifNull(dr("deleted"), 0)) Then
                    qry = ATGenerateQuery("Insert", "trans_sub_contract_offset", fields, Environment.MachineName, Environment.UserName, main.log_id, fields("offset_sub_contract_id"), "offset_sub_contract_id")
                End If
            Else
                If CBool(ifNull(dr("deleted"), 0)) Then
                    qry = ATGenerateQuery("Remove", "trans_sub_contract_offset", Nothing, Environment.MachineName, Environment.UserName, main.log_id, fields("offset_sub_contract_id") & "|", "offset_sub_contract_id")
                Else
                    qry = ATGenerateQuery("Update", "trans_sub_contract_offset", fields, Environment.MachineName, Environment.UserName, main.log_id, fields("offset_sub_contract_id"), "offset_sub_contract_id")
                End If
            End If
            If qry <> "" Then
                dt = Query(qry)
        ...
        Try
            If ds.Tables("checks").Rows.Count > 0 Then
                For i = 0 To ds.Tables("checks").Rows.Count - 1
                    ...
                    fields.Add("isPDC", IIf(dr("isPDC"), "1", "0"))
                    fields.Add("isCash", IIf(dr("isCash"), "1", "0"))
                    ...
                    If hasError Then
                        Dim _tmpstr = ""
                        For Each _fld In fields
                            _tmpstr = String.Format(",[{0} = '{1}']", _fld.Key, _fld.Value)
                        Next
                        MyDebugging.AddText("Error Encountered while saving the check for payment [Check Voucher] : " & _tmpstr)
                    End If
        ...
        Catch ex As Exception
            MyDebugging.AddText(ex.Message)
            MsgBox("An error was encountered while saving cash/check entry." & vbNewLine & "After the saving of this transaction, review the transaction and re entry again the cash/check entry if possible." & vbNewLine &
            "Screenshot and Email to our office." & vbNewLine & ex.Message)
        End Try
        ...
        If ds.Tables("input_vat").Rows.Count > 0 Then
            For i = 0 To ds.Tables("input_vat").Rows.Count - 1
                ...
                fields.Add("exchange_rate", dr("exchange_rate").ToString)
                fields.Add("based_rate", dr("based_rate").ToString)
                fields.Add("for_importation", dr("for_importation").ToString)
        ...
        'input vat for importation
        If ds.Tables("input_vat_for_importation").Rows.Count > 0 Then
            For i = 0 To ds.Tables("input_vat_for_importation").Rows.Count - 1
                qry = ""
                Dim dr As DataRow = ds.Tables("input_vat_for_importation").Rows(i)
                Dim vat_offset_ As Int32
                If dr("offset_type").ToString = "False" Then
                    vat_offset_ = 0
                Else
                    vat_offset_ = 1
                End If
                ...
                fields.Add("jbooks_id", jbooks_ids(dr("jbooks_id").ToString))
                fields.Add("general_id", dr("general_id").ToString)
                fields.Add("offset_type", vat_offset_)
                fields.Add("vat_class", dr("vat_class").ToString)
                fields.Add("vat_type", dr("vat_type").ToString)
                fields.Add("vat_rate", dr("vat_rate").ToString)
                'fields.Add("gross_amount", dr("gross_amount").ToString)
                ...
                Else
                    ...
                    End If
                End If
                Dim input_vat_id_temp As Integer
                If qry <> "" Then
                    dt = Query(qry)
                    input_vat_id_temp = dt.Rows(0)(0).ToString
                End If
                qry = ""
                'input_vat_for_importation_fields
                Dim input_vat_importation As New Dictionary(Of String, String)
                input_vat_importation.Add("import_entry_id", dr("import_entry_id").ToString)
                input_vat_importation.Add("import_entry_no", dr("import_entry_no").ToString)
                input_vat_importation.Add("OR_number", dr("OR_number").ToString)
                input_vat_importation.Add("Total_Landed_Cost", dr("Total_Landed_Cost").ToString)
                input_vat_importation.Add("Taxable_goods", dr("Taxable_goods").ToString)
                input_vat_importation.Add("Date_of_importation", Convert.ToDateTime(dr("Date_of_importation")).ToString("yyyy-MM-dd"))
                input_vat_importation.Add("Dutiable_value", dr("Dutiable_value").ToString)
                input_vat_importation.Add("date_of_vatpayment", Convert.ToDateTime(dr("date_of_vatpayment")).ToString("yyyy-MM-dd"))
                input_vat_importation.Add("country_of_origin", dr("country_of_origin").ToString)
                input_vat_importation.Add("Release_Date", Convert.ToDateTime(dr("Release_Date")).ToString("yyyy-MM-dd"))
                input_vat_importation.Add("vat_amount", dr("vat_amount").ToString)

                input_vat_importation.Add("all_charges_before_release_from_customs", dr("all_charges_before_release_from_customs").ToString)
                input_vat_importation.Add("Exempt", dr("Exempt").ToString)

                input_vat_importation.Add("input_vat_id", input_vat_id_temp.ToString)
                If dr("input_vat_id").ToString = "" Then
                    If dr("deleted").ToString = "False" Then
                        'qry = genInsert("trans_input_vat", fields)
                        qry = ATGenerateQuery("Insert", "trans_input_vat_importation", input_vat_importation, Environment.MachineName, Environment.UserName, main.log_id, dt.ToString, "input_vat_id")
                    End If
                Else
                    If dr("deleted").ToString = "True" Then
                        'qry = String.Format("DELETE FROM trans_input_vat WHERE input_vat_id = {0}", dr("input_vat_id").ToString)
                        qry = ATGenerateQuery("Remove", "trans_input_vat_importation", Nothing, Environment.MachineName, Environment.UserName, main.log_id, dt.ToString & "|", "input_vat_id")
                    Else
                        'qry = genUpdate("trans_input_vat", fields)
                        qry = ATGenerateQuery("Update", "trans_input_vat_importation", input_vat_importation, Environment.MachineName, Environment.UserName, main.log_id, dt.ToString, "input_vat_id")
                    End If
                End If
                If qry <> "" Then
                    dt = Query(qry)
    ...
    Private Sub btnNotSave_Click(sender As Object, e As EventArgs) Handles btnNotSave.Click
        ...
        transmodedet = ""
        loadLastTransaction()
        report_cv_erc_filter.btnInsert.Enabled = False
        report_cv_erc_filter.btnUpdate.Enabled = False
        report_cv_erc_filter.btnRemove.Enabled = False
    End Sub
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        ...
        If selectedRows.Count > 0 Then
            ...
            If selectedRow.Cells("offset").Value > 0 Then
                If "1456789".Contains(selectedRow.Cells("validation").Value.ToString) Then
                    enable_control_det()
                    txt_account_id.Focus()
                    btnOK.Focus()
                    entryValidated = False
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        If "456789".Contains(v.ToString) Then
            ...
            '        If Not entryValidated And v.ToString() <> "1" Then
            '            EntryValidate.NewValidateEntry(Me, dgjbooks, ds, True, , , BookID)
            '        End If
            '    End If
            'End If
        ElseIf "1".Contains(v.ToString) Then
            If CDec(ifNull(txtDebit.EditValue, 0)) = 0 Then
                If Not entryValidated Then
                    EntryValidate.NewValidateEntry(Me, dgjbooks, ds, True, , , BookID)
                End If
        ...
        If v.ToString = "1" Then
            If ds.Tables("jbooks").Select("validation = '1'").Count > 0 Then
                Dim tmp_debit = ds.Tables("jbooks").Compute("sum(credit)", "validation = '1' AND deleted = 'False' AND noCompute = 'False'")
                ...
                If CDec(txtAmountBased.EditValue) < 0 Then
                    txt_amount_in_word.Text = "NEGATIVE " & amount_words(-1 * CDec(txtAmountBased.EditValue), main.DecimalPlaces)
                ElseIf CDec(txtAmountBased.EditValue) > 0 Then
                    txt_amount_in_word.Text = amount_words(FormatNumber(txtAmountBased.EditValue, 10), main.DecimalPlaces)
                Else
                    txt_amount_in_word.Text = ""
                End If
            End If
        ...
        End Select
        If "1457".Contains(v.ToString) Then
        ...
        If txtCredit.Enabled Then
            ...
            If txtCredit.EditValue <> 0 Then
                ...
                If Not txt_account_id.RowData Is Nothing Then
                    ...
                    If v.ToString = "1" Then
                        entryValidated = False
                        NewValidateEntry(Me, dgjbooks, ds, True, BookCode, JournalCode, BookID)
                    End If
                End If
            End If
            If Not txt_account_id.RowData Is Nothing Then
    ...
    Private Sub txtdnref_Leave(sender As Object, e As EventArgs) Handles txt_ref_jbooks_id.Leave
        If txt_ref_jbooks_id.Enabled Then
            ...
            If v.ToString = "1" Then
                If ds.Tables("jbooks").Select("validation = '1'").Count > 0 Then
                    Dim tmp_debit = ds.Tables("jbooks").Compute("sum(credit)", "validation = '1' AND deleted = 'False'")
                    txtAmount.EditValue = If(tmp_debit.ToString = "", 0, tmp_debit)
                    txtAmountBased.EditValue = txtAmount.EditValue * (txtExchangeRate.EditValue / txtBasedRate.EditValue)
                    If CDec(txtAmountBased.EditValue) < 0 Then
                        txt_amount_in_word.Text = "NEGATIVE " & amount_words(-1 * CDec(txtAmountBased.EditValue), main.DecimalPlaces)
    ...
    Private Sub txtday_TextChange(sender As Object, e As EventArgs) Handles txt_day.TextChanged
        ...
        Dim year As Integer = If(txt_year.Value = "", 0, txt_year.Value)
    ...
    Private Sub btnParticular_Click(sender As Object, e As EventArgs) Handles btnParticular.Click
        ...
        trans_particular.TransactionName = "Check Voucher"
            ...
            If myDataTable IsNot Nothing Then
                If myDataTable.Rows.Count > 0 Then
                    ...
                    If currentIndex > 0 Then
                        ...
                        NewControls.refreshRecordDet(dgjbooks, ds, txt_trans_id)
                        If CDec(ifNull(txtAmount.EditValue, 0)) < 0 Then
                            txt_amount_in_word.Text = "NEGATIVE " & amount_words(-1 * CDec(txtAmount.EditValue), main.DecimalPlaces)
                        Else
                            txt_amount_in_word.Text = amount_words(ifNull(txtAmount.EditValue, 0), main.DecimalPlaces)
                        End If
                        computeTotal()
    ...
    Private Sub transactionap_KeyUp(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        ...
        dicShortCut.Add(122, btnCashEntry) 'F11
...
#End Region
    ...
    Private Sub btnCashEntry_Click(sender As Object, e As EventArgs) Handles btnCashEntry.Click
        Dim needCashCheckEntry = ds.Tables("jbooks").Select("validation = 1 AND credit > 0")
        If needCashCheckEntry.Length = 0 Then
            MsgBox("No Cash/Check Entry Found")
        ElseIf needCashCheckEntry.Length = 1 Then
            btnEdit.PerformClick()
            If ShowCashCheckEntry(needCashCheckEntry(0)) Then
                btnSave.PerformClick()
            Else
                btnNotSave.PerformClick()
            End If
        Else
            Dim tmp_dt As New DataTable
            tmp_dt = ds.Tables("jbooks").Clone
            tmp_dt.Clear()
            For Each row As DataRow In needCashCheckEntry
                tmp_dt.ImportRow(row)
            Next
            ChooseCheck.dgAccount.AutoGenerateColumns = False
            ChooseCheck.dgAccount.DataSource = tmp_dt
            ChooseCheck.ShowDialog()
            ChooseCheck.dgAccount.Rows(0).Selected = True
            If ChooseCheck.DialogResult = Windows.Forms.DialogResult.OK Then
                Dim selectedID = ChooseCheck.SelectedID
                Dim selectedRow = tmp_dt.Select(String.Format("jbooks_id_tmp = '{0}'", selectedID))
                If selectedRow.Length > 0 Then
                    btnEdit.PerformClick()
                    If ShowCashCheckEntry(selectedRow(0)) Then
                        btnSave.PerformClick()
                    Else
                        btnNotSave.PerformClick()
                    End If
        ...
        End If
    End Sub
    Private Function ShowCashCheckEntry(ByVal MyDataRow As DataRow) As Boolean
        Dim isSave As Boolean = False
        Dim MyJBooksID = MyDataRow.Item("jbooks_id_tmp").ToString
        Dim MyAccountCode = MyDataRow.Item("account_code").ToString
        Dim MyGeneralID = txt_general_id.Value
        Dim MyGeneralCode = txt_general_id.Text
        Dim MyGeneralName = txt_general_name.Text
        Dim MyCurrencyID = MyDataRow.Item("currency_id").ToString
        Dim MyExchangeRate = MyDataRow.Item("exchange_rate").ToString
        Dim MyBasedRate = MyDataRow.Item("based_rate").ToString
        Dim MyCredit = MyDataRow.Item("credit").ToString
        Dim MyDebit = MyDataRow.Item("debit").ToString

        'Dim accountname = txtaccountname.Text
        Dim tmp_dr() As DataRow = ds.Tables("checks").Select("jbooks_id LIKE '" & MyJBooksID & "%'")
        Dim tmp_dt As New DataTable
        ...
        cashcv.jbooks_id = MyJBooksID
        cashcv.genref_id = MyGeneralID
        cashcv.genref_code = MyGeneralCode
        cashcv.genref_name = MyGeneralName
        cashcv.currency_id = MyCurrencyID
        cashcv.exchange_rate = MyExchangeRate
        cashcv.based_rate = MyBasedRate
        cashcv.journal_code = JournalCode
        cashcv.book_code = BookCode
        Dim trans_date As Date = New Date(txt_year.Value, txt_month.Value, txt_day.Text)
        cashcv.trans_date = trans_date
        Dim sql As String = String.Format("SELECT bank_id,bank_code,bank_name " &
            "FROM lib_erp_bank " &
            "WHERE bank_code = '{0}'", MyAccountCode)
        Dim dt As DataTable = FsQuery(sql)
        If dt.Rows.Count > 0 Then
        ...
        End If
        'cashcv.txtcheckamount.NoClear = True
        'cashcv.txt_check_amount_based.NoClear = True
        cashcv.OutstandingBalance = MyCredit
        cashcv.txtCheckAmount.EditValue = MyCredit
        cashcv.txtCheckAmountBased.EditValue = cashcv.txtCheckAmount.EditValue * (MyExchangeRate / MyBasedRate)
        'raymond - 2016-06-08 (start)
        Dim sql_trans_jbooks As String = String.Format("SELECT b.ref_no,c.trans_no FROM trans_jbooks a LEFT JOIN trans_jbooks_ref b ON b.jbooks_id = a.jbooks_id LEFT JOIN trans_header c ON c.trans_id = a.trans_id WHERE a.jbooks_id = {0}", MyJBooksID)
        ...
        End If
        MyCredit = 0
        If ds.Tables("checks").Select("jbooks_id LIKE '" & MyJBooksID & "%' AND deleted = 'False'").Length > 0 Then
            MyCredit = ds.Tables("checks").Compute("Sum(check_amount)", "jbooks_id LIKE '" & MyJBooksID & "%' AND deleted = 'False'")
            MyDebit = 0
        End If
        Return isSave
    End Function
    ...
    Private Sub btnGetAp_Click(sender As Object, e As EventArgs) Handles btnGetAp.Click

        If ifNull(txtExchangeRate.EditValue, 0) = 0 Or ifNull(txtBasedRate.EditValue, 0) = 0 Then
            MsgBox("No currency setup.")
            Exit Sub
        End If
        Dim account_id As String = ""
        Dim temporary_id As String = "tmp_" & GenerateRandomString()
        Dim dr_account As DataRow = Nothing
        For Each dr As DataRow In ds.Tables("jbooks").Rows
            If ifNull(dr.Item("validation"), "0").ToString = "1" Then
                account_id = dr.Item("account_id").ToString
                temporary_id = dr.Item("jbooks_id").ToString
                Dim sql_account As String = String.Format("SELECT account_id,account_code,account_name FROM lib_erp_chart WHERE account_id = '{0}'", account_id)
                Dim dt_account As DataTable = FsQuery(sql_account)
                If dt_account.Rows.Count > 0 Then
                    dr_account = dt_account.Rows(0)
                End If
            End If
        Next
        Dim tmp_dt As New DataTable
        Dim cash_dt As New DataTable
        For Each col As DataColumn In ds.Tables("jbooks").Columns
            tmp_dt.Columns.Add(col.ColumnName)
        Next
        For Each col As DataColumn In ds.Tables("checks").Columns
            cash_dt.Columns.Add(col.ColumnName)
        Next
        For i As Integer = 0 To ds.Tables("jbooks").Rows.Count - 1
            tmp_dt.ImportRow(ds.Tables("jbooks").Rows(i))
        Next
        For i As Integer = 0 To ds.Tables("checks").Rows.Count - 1
            cash_dt.ImportRow(ds.Tables("checks").Rows(i))
        Next
        Dim offset_ds As DataTable = GetAPForm.ds.Tables("jbooks")
        Dim cash_ds As DataTable = GetAPForm.ds.Tables("checks")
        offset_ds.Clear()
        cash_ds.Clear()
        GetAPForm.bank_id = account_id
        GetAPForm.RowData = dr_account
        GetAPForm.book_id = BookID
        GetAPForm.trans_id = txt_trans_id.Text
        GetAPForm.filter_date = New Date(txt_year.Text, txt_month.Value, txt_day.Text)
        GetAPForm.currency_id = txt_currency_id.Value
        GetAPForm.jbooks_id = ""
        GetAPForm.GeneralID = txt_general_id.Value
        GetAPForm.GeneralCode = txt_general_id.Text
        GetAPForm.GeneralName = txt_general_name.Text
        GetAPForm.ExchangeRate = txtExchangeRate.EditValue
        GetAPForm.BasedRate = txtBasedRate.EditValue
        GetAPForm.InitializeData()
        dtReplace(offset_ds, tmp_dt, "ref_jbooks_id")
        dtCopy(cash_ds, cash_dt, {"check_id", "check_no"})
        If GetAPForm.ds.Tables("jbooks").Rows.Count > 0 Then
            If GetAPForm.ShowDialog() = DialogResult.OK Then
                'Dim bank_dr As DataRow = GetAPForm.txt_bank_id.RowData
                'Dim bank_id As String = GetAPForm.txt_bank_id.Value
                Dim foundDRS As DataRow() = ds.Tables("jbooks").Select(String.Format("account_id = '{0}' AND credit <> 0", account_id))
                If foundDRS.Length = 0 Then
                    Dim newDR As DataRow = ds.Tables("jbooks").NewRow
                    newDR.Item("jbooks_id") = temporary_id
                    newDR.Item("account_id") = GetAPForm.txt_bank_id.RowData.Item("account_id").ToString
                    newDR.Item("account_code") = GetAPForm.txt_bank_id.RowData.Item("account_code").ToString
                    newDR.Item("account_name") = GetAPForm.txt_bank_id.RowData.Item("account_name").ToString
                    newDR.Item("currency_id") = GetAPForm.currency_id
                    newDR.Item("exchange_rate") = GetAPForm.ExchangeRate
                    newDR.Item("based_rate") = GetAPForm.BasedRate
                    newDR.Item("debit") = 0
                    newDR.Item("credit") = GetAPForm.txttotal.DecimalValue
                    newDR.Item("debit_based") = 0
                    newDR.Item("credit_based") = GetAPForm.txttotal.DecimalValue * (GetAPForm.ExchangeRate / GetAPForm.BasedRate)
                    newDR.Item("trans_id") = GetAPForm.trans_id
                    newDR.Item("validation") = "1"
                    ds.Tables("jbooks").Rows.Add(newDR)
                End If
                dtCopy(ds.Tables("jbooks"), GetAPForm.ds.Tables("jbooks"), {"ref_jbooks_id"}, "isInputted", "jbooks_id")
                dtCopy(ds.Tables("checks"), GetAPForm.ds.Tables("checks"), {"check_id", "check_no"})
                For Each dr As DataRow In ds.Tables("jbooks").Rows
        ...
        Next
        'dgjbooks.DataSource = newDt
        loadDTToDG(dgjbooks, newDt)
    End Sub

    Private Sub transactioncv_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'transID = txt_trans_id.Text
    ...
    Private Sub btn_particularOption_Click(sender As Object, e As EventArgs) Handles btn_particularOption.Click
        trans_detailed_particulars.TransactionDetailedID = txt_trans_id.Text
        trans_detailed_particulars.dsDetailedParticular.Tables("trans_header_detailed_particular").Clear()
        copyDataTable(trans_detailed_particulars.dsDetailedParticular.Tables("trans_header_detailed_particular"), ds.Tables("detailed_particular"), "trans_detailed_particular_id")

        trans_detailed_particulars.EnableParticular(Not btnAdd.Visible)
        Dim dt_det As DataTable = trans_detailed_particulars.dsDetailedParticular.Tables("trans_header_detailed_particular")
        trans_detailed_particulars.ShowDialog()
        trans_detailed_particulars.dgMainDetailed.Enabled = True
        copyDataTable(ds.Tables("detailed_particular"), trans_detailed_particulars.dsDetailedParticular.Tables("trans_header_detailed_particular"), "trans_detailed_particular_id")
    End Sub
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        'report_cv_erc_filter.TransactionID = txt_trans_id.Text
        'report_cv_erc_filter.ds.Tables("trans_mfo_pap").Clear()
        'copyDataTable(report_cv_erc_filter.ds.Tables("trans_mfo_pap"), ds.Tables("trans_mfo_pap"), "mfo_pap_id")
        'report_cv_erc_filter.ds.Tables("trans_additional").Clear()
        'copyDataTable(report_cv_erc_filter.ds.Tables("trans_additional"), ds.Tables("trans_additional"), "trans_additional_id")
        report_cv_erc_filter.ShowDialog()
        'copyDataTable(ds.Tables("trans_mfo_pap"), report_cv_erc_filter.ds.Tables("trans_mfo_pap"), "mfo_pap_id")
        'copyDataTable(ds.Tables("trans_additional"), report_cv_erc_filter.ds.Tables("trans_additional"), "trans_additional_id")
    End Sub
    ...
    Private Sub txt_trans_id_TextChanged(sender As Object, e As EventArgs) Handles txt_trans_id.TextChanged
        'initLoad()
    End Sub
    Public Sub initLoad()
        'Dim frm_rep_erc = report_cv_erc_filter
        '''trans_mfo_pap
        'If txt_trans_id.Text <> "" Then
        '    Dim sql As String = String.Format("SELECT * FROM trans_mfo_pap WHERE trans_id = {0}", txt_trans_id.Text)
        '    frm_rep_erc.ds.Tables("trans_mfo_pap").Clear()
        '    frm_rep_erc.MyDT = frm_rep_erc.ds.Tables("trans_mfo_pap").Clone
        '    frm_rep_erc.MyDT.Clear()

        '    Dim dt As DataTable = Query(sql)
        '    For Each row As DataRow In dt.Rows
        '        frm_rep_erc.ds.Tables("trans_mfo_pap").ImportRow(row)
        '        frm_rep_erc.MyDT.ImportRow(row)
        '    Next
        '    frm_rep_erc.dgMFO.DataSource = frm_rep_erc.MyDT

        '    ''trans_additional
        '    Dim sql_additional As String = String.Format("select * from trans_additional a left join {0}.lib_erp_fund_cluster b on b.fund_code = a.fund_cluster WHERE a.trans_id = {1}", _serverDBFS, txt_trans_id.Text)
        '    frm_rep_erc.ds.Tables("additional").Clear()
        '    frm_rep_erc.MyDT = frm_rep_erc.ds.Tables("additional").Clone
        '    frm_rep_erc.MyDT.Clear()

        '    Dim dt_addtional As DataTable = Query(sql_additional)
        '    For Each row As DataRow In dt_addtional.Rows
        '        frm_rep_erc.ds.Tables("additional").ImportRow(row)
        '        frm_rep_erc.MyDT.ImportRow(row)
        '    Next

        '    Dim dr As DataRow = Nothing
        '    If frm_rep_erc.ds.Tables("additional").Rows.Count <> 0 Then
        '        dr = frm_rep_erc.ds.Tables("additional").Rows(0)

        '        frm_rep_erc.txt_ors_burs.Text = dr("ors_burs_no").ToString
        '        frm_rep_erc.txt_fund_code.Text = dr("fund_cluster").ToString
        '        frm_rep_erc.txt_mode_of_payment.Text = dr("mode_of_payment").ToString
        '        frm_rep_erc.txt_fund_name.Text = dr("fund_name").ToString

        '        If dr("mode_of_payment") = "MDS Check" Then
        '            frm_rep_erc.rdo_mds_check.Checked = True
        '        ElseIf dr("mode_of_payment") = "Commercial Check" Then
        '            frm_rep_erc.rdo_commercial_check.Checked = True
        '        ElseIf dr("mode_of_payment") = "ADA" Then
        '            frm_rep_erc.rdo_ada.Checked = True
        '        ElseIf dr("mode_of_payment") = "Others" Then
        '            frm_rep_erc.rdo_others.Checked = True
        '        End If
        '    End If
        'End If
    End Sub

    Private Sub hdr_Paint(sender As Object, e As PaintEventArgs) Handles hdr.Paint
