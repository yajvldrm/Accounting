...
Public Class pendingAR
    ...
    Public Sub InitializeConnection()
        ...
        Dim dt As DataTable = ExecuteWithParameter("acc_get_pending_ar", ConnectionStringMain, CommandType.StoredProcedure, col)
    ...
    Private Sub pendingAP_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        main.btnShowPendingAR.Visible = False
    ...
    Private Sub pendingAP_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        main.btnShowPendingAR.Visible = True
    End Sub
        ...
        If selectedIDs <> "" Then
            ...
            If pendingSettings.DialogResult = DialogResult.OK Then
                ...
                Else
                    ...
                    col.Add(New KeyValuePair(Of String, String)("@unit_type", unit_type))
                    Dim dtTrans As DataTable = ExecuteWithParameter("get_trans_aror", ConnectionStringMain, CommandType.StoredProcedure, col)
                    Dim dtJBooks As DataTable = ExecuteWithParameter("get_trans_aror_new", ConnectionStringMain, CommandType.StoredProcedure, col)
                    ...
                    If dtTrans.Rows.Count > 0 And dtJBooks.Rows.Count > 0 Then
                        ...
                        totalAmount = dtJBooks.Compute("SUM(credit)", "TRUE")
                        ...
                        newDR.Item("debit") = totalAmount
                        newDR.Item("credit") = 0
                        newDR.Item("debit_based") = (totalAmount * (exchange_rate / based_rate))
                        newDR.Item("credit_based") = 0
                        ...
                        'cashor
                        Dim cashor_ds As DataTable = cashor.ds.Tables("checks")
                        cashor_ds.Clear()
                        'dtCopy(cashor_ds, tmp_dt, {"check_id", "check_no"})
                        cashor.jbooks_id = check_id
                        cashor.genref_id = general_id
                        cashor.genref_code = general_code
                        cashor.genref_name = general_name
                        cashor.currency_id = currency_id
                        cashor.exchange_rate = exchange_rate
                        cashor.based_rate = based_rate
                        cashor.journal_code = "CR"
                        cashor.book_code = book_code
                        ...
                        Else
                            ...
                            cashor.txt_bank_name.NoClear = False
                        End If
                        'cashor.txt_check_amount.NoClear = True
                        'cashor.txt_check_amount_based.NoClear = True
                        cashor.OutstandingBalance = totalAmount
                        cashor.txtCheckAmount.EditValue = totalAmount
                        cashor.txtCheckAmountBased.EditValue = cashor.txtCheckAmount.EditValue * (exchange_rate / based_rate)
                        ...
                        If cashor.ShowDialog() = DialogResult.OK Then
                            Dim tmp_ds = cashor.ds.Tables("checks")
                            dtCopy(dtChecks, cashor.ds.Tables("checks"), {"check_id", "check_no"})
                            transactioncr.CreateORFromAR = True
                            transactioncr.BookID = book_id
                            transactioncr.BookCode = book_code
                            transactioncr.BookName = book_name
                            transactioncr.JournalID = "4"
                            transactioncr.JournalCode = "CV"
                            transactioncr.transMode = "add"
                            transactioncr.mySettings = New settings
                            showForm(transactioncr, main)
                            transactioncr.ReverseAR(dtTrans.Rows(0), dtJBooks, dtChecks)
                        End If
                    Else
