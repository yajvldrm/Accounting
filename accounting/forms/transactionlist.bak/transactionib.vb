...
Public Class transactionib
    ...
    Private Sub clear_control()
        For Each ctrl As Control In hdr.Controls
            If TypeOf ctrl Is user_control.textbox Then
                ...
                If ctr.Name <> "txtmonth" And ctr.Name <> "txtyear" Then
                    ...
                    End If
                End If
            ElseIf TypeOf ctrl Is user_control.MyTextBox Then
                Dim ctr = CType(ctrl, user_control.MyTextBox)
                If ctr.Name <> "txtmonth" And ctr.Name <> "txtyear" Then
                    If ctr.NumberFormat.Substring(0, 1) = "#" Then
                        ctr.DecimalValue = 0.0
                    Else
        ...
        txttransid.Value = 0
        txttransid.Text = 0
        txttransibid.Value = Nothing
        txttransibid.Text = Nothing
    ...
    Friend Sub refresh_record()
        ...
        ",IFNULL(b.trans_ib_id,'') AS trans_ib_id,IFNULL(b.si_no,'') AS si_no,IFNULL(b.dr_no,'') AS dr_no,IFNULL(b.po_no,'') AS po_no " & _
        ",IFNULL(b.terms_id,'') AS terms_id,IFNULL(b.nodays,0) AS nodays,e.terms_name,b.due_date " & _
        "FROM trans_header a " & _
        "LEFT JOIN trans_header_ib b ON a.trans_id = b.trans_id " & _
        ...
        If dt.Rows.Count > 0 Then
            ...
            txttransibid.Text = dt.Rows(0).Item("trans_ib_id").ToString
            ...
            For Each ctrl As Control In hdr.Controls
                If TypeOf ctrl Is user_control.textbox Then
                    ...
                    End If
                ElseIf TypeOf ctrl Is user_control.MyTextBox Then
                    Dim txtbox As user_control.MyTextBox = CType(ctrl, user_control.MyTextBox)
                    If txtbox.ValueSource <> "" Then
                        If dt.Columns.Contains(txtbox.ValueSource) Then
                            txtbox.Value = dt.Rows(0).Item(txtbox.ValueSource).ToString
                            If txtbox.NumberFormat <> "" And txtbox.NumberFormat <> "@" Then
                                Dim tmp_str = dt.Rows(0).Item(txtbox.ValueSource).ToString
                                txtbox.DecimalValue = tmp_str
                            End If
                        End If
                    End If
                    If txtbox.TextSource <> "" Then
    ...
    Private Sub refresh_record_det()
        Dim sql As String = String.Format("select True AS isInputted,a.*,if(a.debit!=0,1,2) as dc,a.jbooks_id as jbooks_id_tmp " & _
            ...
            "left join {0}.lib_erp_project b on a.project_id = b.project_id " & _
            "left join {0}.lib_erp_department c on a.department_id = c.department_id " & _
            "left join {0}.lib_erp_allocation d on a.allocation_id = d.allocation_id " & _
            "left join {0}.lib_erp_chart e on a.account_id = e.account_id " & _
            "left join {0}.lib_erp_general f on a.general_id = f.general_id " & _
            "left join {0}.lib_erp_currency g on a.currency_id = g.currency_id " & _
            "left join {1}.trans_jbooks_ap h on h.jbooks_id = a.jbooks_id " & _
            "left join {1}.trans_jbooks_ar i on i.jbooks_id = a.jbooks_id " & _
            "left join {0}.lib_erp_chart j on j.account_id = a.account_id " & _
            "left join {0}.lib_erp_terms k on k.terms_id = h.terms_id " & _
            "left join {0}.lib_erp_terms l on l.terms_id = i.terms_id " & _
            "left join trans_jbooks a2 on a.ref_jbooks_id = a2.jbooks_id " & _
            "left join trans_header z2 on a2.trans_id = z2.trans_id " & _
            "left join {0}.lib_erp_book x2 on z2.book_id = x2.book_id " & _
            "left join {0}.lib_erp_journal y2 on z2.journal_id = y2.journal_id " & _
            "where a.trans_id = {2} " & _
            "order by dc,e.account_code", _serverDBFS, _serverDBMain, txttransid.Text)
        Dim dt As DataTable = Query(sql)
    ...
    Private Sub bind_control_det()
        ...
        If tmp_dr.Count > 0 Then
            ...
            txtnodaysdet.DecimalValue = dr("nodays")
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        report_ib_filter.journal_id = journal_id
        report_ib_filter.book_id = book_id
        report_ib_filter.transid = txttransid.Text
        report_ib_filter.transno = txttransno.Text
        report_ib_filter.ShowDialog()
    End Sub
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        'ap
        If Not txtdrno.Text = "" Or Not txtpono.Text = "" Or Not txtterms.Text = "" Or Not txtduedate.Text = "" Then
            qry = ""
            fields = New Dictionary(Of String, String)
            fields.Add("trans_ib_id", txttransibid.Text)
            fields.Add("trans_id", trans_id)
            fields.Add("dr_no", txtdrno.Text)
        ...
        'Dim btnFilter As Button = CType(mainList.Controls.Find("btnFilter", True)(0), Button)
        'btnFilter.PerformClick()
        'gridRowFocus(dglist, {pk_name, tmp_pk})
        'mainList.setSelectedRow(tmp_pk, pk_name)
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        If v.ToString = "2" Then
            If ds.Tables("jbooks").Select("validation = '2'").Count > 0 Then
                txtamount.DecimalValue = ds.Tables("jbooks").Compute("sum(debit)", "validation = '2'")
    ...
    Private Sub txtgeneral_Leave(sender As Object, e As EventArgs)
        general = FsQuery("SELECT * FROM lib_erp_general WHERE general_type = 2 OR general_type = 3 ")
    ...
    Private Sub txtcurrency_Leave(sender As Object, e As EventArgs)
        If txtcurrency.RowData IsNot Nothing Then
            Dim dr = txtcurrency.RowData
            txtexchangerate.Text = dr.Item("exchange_rate")
            txtbasedrate.Text = dr.Item("based_rate")
            txt_currency_id.RowData = dr
            txt_currency_id.Value = dr.Item("currency_id")
            txt_currency_id.Text = dr.Item("currency_name")
            txt_exchange_rate.DecimalValue = dr.Item("exchange_rate")
            txt_based_rate.DecimalValue = dr.Item("based_rate")
        End If
    End Sub

    Private Sub txt_currency_id_Leave(sender As Object, e As EventArgs)
    ...
    Private Sub txtaccount_Leave(sender As Object, e As EventArgs)
    ...
    Private Sub txtproject_Leave(sender As Object, e As EventArgs)
    ...
    Private Sub txtdepartment_Leave(sender As Object, e As EventArgs)
    ...
    Private Sub txtallocation_Leave(sender As Object, e As EventArgs)
    ...
    Private Sub txttermsiddet_Leave(sender As Object, e As EventArgs)
    ...
    Private Sub txtgeneraldet_Leave(sender As Object, e As EventArgs)
        Dim v As Integer = txtaccount.RowData("validation")
        If "2310".IndexOf(v.ToString) > 0 Then
            general = FsQuery("SELECT * FROM lib_erp_general WHERE general_type = 2 or general_type = 3 ")
            If general.Rows.Count = 0 Then
    ...
    Private Sub txtday_TextChange(sender As Object, e As EventArgs)
        Dim month As Integer = If(txtmonth.Value = "", 0, txtmonth.Value)
        Dim year As Integer = If(txtyear.Value = "", 0, txtyear.Value)
    ...
    Private Sub txtterms_Leave(sender As Object, e As EventArgs)
        terms = FsQuery("select * from lib_erp_terms where terms_type = 1 or terms_type = 3")
        If terms.Rows.Count = 0 Then
            MsgBox("No record found.")
        Else
            ...
            If terms.Rows.Count = 1 Then
                dr = terms.Rows(0)
            Else
                Dim dra() As DataRow = terms.Select("terms_name like '" & txtterms.Text & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = terms
    ...
    Private Sub dgjbooks_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles dgjbooks.CellFormatting
        ...
        '    If e.RowIndex <= ds.Tables("jbooks").Rows.Count - 1 Then
        '        drv = ds.Tables("jbooks").DefaultView.Item(e.RowIndex)
        '        Dim c As Color
    ...
    Private Sub btnParticular_Click(sender As Object, e As EventArgs) Handles btnParticular.Click
        Dim frmParticular As New trans_particular
        frmParticular.trans_id = txttransid.Text
        frmParticular.strparticular = txtdesc.Text
        frmParticular.transaction_name = "Sales Voucher"
        For Each dr As DataRow In ds.Tables("particular").Rows
            frmParticular.ds.Tables("particular").ImportRow(dr)
        Next
        frmParticular.ShowDialog()
    End Sub

    Private Sub txt_currency_id_Leave_1(sender As Object, e As EventArgs) Handles txt_currency_id.Leave
    ...
    Private Sub txtterms_Leave_1(sender As Object, e As EventArgs) Handles txtterms.Leave
        If txtterms.RowData IsNot Nothing Then
            Dim dr = txtterms.RowData
            txtnodays.Enabled = If(dr.Item("fixed_day"), True, False)
            txtnodays.DecimalValue = dr.Item("nodays")
        End If
    End Sub

    Private Sub txttermsiddet_Leave_1(sender As Object, e As EventArgs) Handles txttermsiddet.Leave
        If txttermsiddet.RowData IsNot Nothing Then
            Dim dr = txttermsiddet.RowData
            txtnodaysdet.Enabled = If(dr.Item("fixed_day"), True, False)
            txtnodaysdet.DecimalValue = dr.Item("nodays")
        End If
    End Sub
End Class