...
Public Class apagingsummary
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click, btnExcel.Click
        ...
        Select Case account
            ...
            Case "2"
                ...
                If account_id = "" Then
                    MsgBox("Setup Multiple Accounts First.")
                    Exit Sub
                End If
            Case "3"
                If account_id = "" Then
        ...
        Dim _name As String = "Aging Summary of Account Payable"
    ...
    '    'query
    '    Dim sql As String = String.Format("SELECT b.jbooks_id,a.trans_id,a.trans_no,a.trans_date " &
    ...
    '        ",b.currency_id,k.currency_code,k.currency_name " &
    '        ",a.book_id,_book.book_code,_book.book_name " &
    '        "FROM trans_header a " &
    ...
    '        "LEFT JOIN {0}.lib_erp_currency k ON k.currency_id = b.currency_id " &
    ...
    '    End If
    '    sql = String.Format("SELECT a.trans_id,a.trans_no,a.trans_date " &
    ...
    '        ",a.terms_id,a.terms_code,a.terms_name,a.nodays " &
    '        ",a.journal_id,a.journal_code,a.journal_name " &
    '        ",a.project_id,a.project_code,a.project_name " &
    '        ",a.department_id,a.department_code,a.department_name " &
    '        ",a.account_id,a.account_code,a.account_name, a.currency_code, a.currency_name " &
    '        ",a.book_id,a.book_code,{0} " &
    '        "FROM ( " &
    '        " " & sql &
    '        ") a " &
    '        "", sql_book_select)
    '    sql &= String.Format(" GROUP BY a.general_id ")
    '    If currency_id <> "" Or currency = "2" Then
    '        sql &= String.Format(" ,a.currency_id ")
    '    End If
    '    If groupByBook.Checked Then
    '        sql &= String.Format(",a.book_id ")
    '    End If
    ...
    '    For Each dr_balance As DataRow In dt.Rows
    '        Dim trans_date As Date = CType(dr_balance.Item("trans_date"), Date).AddDays(CType(If(dr_balance.Item("nodays").ToString = "", 0, dr_balance.Item("nodays").ToString), Decimal))
    '        If DateDiff(DateInterval.Day, trans_date, CType(date_to, Date)) < first_day Then
    '            If currency = "1" Then
    ...
    Private Sub generateExcelReport(ByVal listAccount As List(Of Object), ByVal dicHeader As Dictionary(Of String, String), ByVal strCol As String, ByVal dtAccount As DataTable, ByVal dtAging As DataTable)
        ...
        xlWorkSheet.Range(xlWorkSheet.Cells(current_row, 1), xlWorkSheet.Cells(current_row, 8)).Interior.Color = System.Drawing.Color.FromArgb(215, 228, 242)
        ...
        xlWorkSheet.Cells(current_row, 8).Borders(Excel.XlBordersIndex.xlEdgeRight).LineStyle = Excel.XlLineStyle.xlContinuous
        xlWorkSheet.Cells(current_row, current_column).ColumnWidth = 15
        ...
        xlWorkSheet.Cells(current_row, 1).Font.Italic = True
        xlWorkSheet.Cells(current_row, 1).IndentLevel = 5
        xlWorkSheet.Cells(current_row, 2).Value = String.Format("=SUBTOTAL(9,B{0}:B{1})", first_row, current_row - 1)
        xlWorkSheet.Cells(current_row, 3).Value = String.Format("=SUBTOTAL(9,C{0}:C{1})", first_row, current_row - 1)
        xlWorkSheet.Cells(current_row, 4).Value = String.Format("=SUBTOTAL(9,D{0}:D{1})", first_row, current_row - 1)
        xlWorkSheet.Cells(current_row, 5).Value = String.Format("=SUBTOTAL(9,E{0}:E{1})", first_row, current_row - 1)
        xlWorkSheet.Cells(current_row, 6).Value = String.Format("=SUBTOTAL(9,F{0}:F{1})", first_row, current_row - 1)
        ...
        xlWorkSheet.Cells(current_row, 1).Font.Italic = True
        xlWorkSheet.Cells(current_row, 1).IndentLevel = 5
        xlWorkSheet.Cells(current_row, 3).Value = String.Format("=SUM(C{0}/B{0})", current_row - 1)
        xlWorkSheet.Cells(current_row, 3).HorizontalAlignment = Excel.Constants.xlRight
        xlWorkSheet.Cells(current_row, 3).NumberFormat = "##0.00%"
        xlWorkSheet.Cells(current_row, 4).Value = String.Format("=SUM(D{0}/B{0})", current_row - 1)
        xlWorkSheet.Cells(current_row, 4).HorizontalAlignment = Excel.Constants.xlRight
        xlWorkSheet.Cells(current_row, 4).NumberFormat = "##0.00%"
        xlWorkSheet.Cells(current_row, 5).Value = String.Format("=SUM(E{0}/B{0})", current_row - 1)
    ...
    Public Sub generateColumn(ByVal rep As XtraReport, ByVal dt As DataTable, ByVal first_day As String, ByVal last_day As String, Optional col_prefix As String = "")
        ...
        cellFooter3Current.Summary = _summary()
        ...
        For Each row As DataRow In dt.Rows
            ...
            cellFooter3Column.Summary = _summary()
        ...
        cellFooter3Over.Summary = _summary()
