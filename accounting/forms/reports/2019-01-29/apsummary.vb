...
Public Class apsummary
    ...
    Private Sub txt_currency_id_Leave(sender As Object, e As EventArgs) Handles txt_currency_id.Leave
        ...
        Else
            ...
            Else
                Dim dra() As DataRow = data_currency.Select("currency_code = '" & txt_currency_id.Text.ToString.Replace("'", "''") & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
    ...
    Private Sub txt_project_id_Leave(sender As Object, e As EventArgs) Handles txt_project_id.Leave
        ...
        Else
            ...
            If data.Rows.Count = 1 Then
                dr = data.Rows(0)
            Else
                Dim dra() As DataRow = data.Select("project_code = '" & txt_project_id.Text.ToString.Replace("'", "''") & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = data
    ...
    Private Sub txt_department_id_Leave(sender As Object, e As EventArgs) Handles txt_department_id.Leave
        ...
        Else
            ...
            If data.Rows.Count = 1 Then
                dr = data.Rows(0)
            Else
                Dim dra() As DataRow = data.Select("department_code = '" & txt_department_id.Text.ToString.Replace("'", "''") & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = data
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click, btnExcel.Click
        ...
        Dim sql As String = String.Format("SELECT e.account_id,a.trans_id,a.trans_no,a.trans_date,a.description " &
                                          ...
                                          "LEFT JOIN (SELECT a.* FROM {0}.lib_erp_chart a WHERE a.validation = 3) f ON f.account_id = b.account_id " &
                                          ...
                                          "LEFT JOIN {0}.lib_erp_department l ON l.department_id = b.department_id " &
                                          "LEFT JOIN {0}.lib_erp_book n ON n.book_id = a.book_id " &
        ...
        Select Case account
            ...
            Case "3"
                sql &= String.Format(" AND e.account_id = {0} ", account_id)
        End Select
        ...
        sql &= String.Format(" AND e.validation = 3 AND m.company_id = '{0}' ", main.Sys_Default.Rows(0).Item("company_id").ToString)
    ...
    Private Sub generateExcelReport(ByVal listAccount As List(Of Object), ByVal colName As String, ByVal dicHeader As Dictionary(Of String, String), ByVal dt As DataTable, Optional originalCurrency As Boolean = False, Optional compare As Boolean = False)
        ...
        xlWorkSheet.Cells(2, 1) = "SUMMARY OF ACCOUNTS PAYABLE"
