...
Public Class incomestatement
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs, str_sort As String)
        ...
        Dim orderByName As Boolean = rdoByName.Checked
        ...
        If groupByBook.Checked Then
            sql_balance &= String.Format("SELECT a.account_id,a.account_code,a.account_name,b.chart_type " &
                ...
                "	,IF(SUM(IFNULL(a.debit,0)) > SUM(IFNULL(a.credit,0)),SUM(IFNULL(a.debit,0)) - SUM(IFNULL(a.credit,0)),0) AS debit " &
                "	,IF(SUM(IFNULL(a.credit,0)) > SUM(IFNULL(a.debit,0)),SUM(IFNULL(a.credit,0)) - SUM(IFNULL(a.debit,0)),0) AS credit " &
                "	,IF(SUM(IFNULL(a.debit_based,0)) > SUM(IFNULL(a.credit_based,0)),SUM(IFNULL(a.debit_based,0)) - SUM(IFNULL(a.credit_based,0)),0) AS debit_based " &
                "	,IF(SUM(IFNULL(a.credit_based,0)) > SUM(IFNULL(a.debit_based,0)),SUM(IFNULL(a.credit_based,0)) - SUM(IFNULL(a.debit_based,0)),0) AS credit_based " &
                "	FROM {0}.SelectChartAmount a " &
                "	LEFT JOIN {0}.SelectIncomeExpenseChart b ON b.account_id = a.account_id " &
                "	WHERE DATE_FORMAT(a.trans_date,'%m-%Y') = DATE_FORMAT('{1}','%m-%Y') AND a.book_id IN ({5}) " &
                ...
                "	FROM {0}.SelectChartAmount a " &
                "	LEFT JOIN {0}.SelectIncomeExpenseChart b ON b.account_id = a.account_id " &
                "	WHERE DATE_FORMAT(a.trans_date,'%m-%Y') = DATE_FORMAT('{3}','%m-%Y') AND a.book_id IN ({5}) " &
                "	GROUP BY a.book_id,b.account_id " &
                ...
                "	FROM {0}.SelectChartAmount a " &
                "	LEFT JOIN {0}.SelectIncomeExpenseChart b ON b.account_id = a.account_id " &
                "	WHERE a.trans_date <= '{2}' AND a.book_id IN ({5}) " &
                "	GROUP BY a.book_id,b.account_id " &
                ") g ON g.account_id = a.account_id AND g.book_id = _book.book_id " &
                "WHERE b.chart_type IN ('I','E') AND _book.book_id IN ({5}) ", _serverDBMain, date_from_1.ToString("yyyy-MM-dd"), date_from.ToString("yyyy-MM-dd"), date_to_1.ToString("yyyy-MM-dd"), date_to.ToString("yyyy-MM-dd"), bookIds.Substring(1))
        Else
            sql_balance = String.Format("SELECT a.account_id,a.account_code,a.account_name,b.chart_type " &
                ",IF(b.chart_type = 'I','INCOME','EXPENSE') AS chart_type_name " &
                ",a.chart_class_id,b.chart_class_code,b.chart_class_name " &
        ...
        End If
        sql_balance &= String.Format("ORDER BY FIELD(b.chart_type,'I','E'),d.chart_group_name ASC ")
        If orderByName Then
            sql_balance &= String.Format(",a.account_name")
        Else
            sql_balance &= String.Format(",a.account_code")
        End If
