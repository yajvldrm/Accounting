...
Public Class inputvat
    Private module_code As String = "ACC01015"
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click, btnExcel.Click
        ...
        Select Case general
            ...
            Case "2"
                Dim sql_general_id As String = String.Format("SELECT GROUP_CONCAT(a.general_id) AS general_id " & _
                    "FROM lib_acc_general_multipe a " & _
        ...
        Dim sql As String = String.Format("SELECT a.trans_id,a.trans_no,a.trans_date " &
            ",b.jbooks_id " &
            ",c.input_vat_id AS vat_id,c.gross_amount,c.vat_amount,c.net_amount,c.gross_amount_based,c.vat_amount_based,c.net_amount_based " &
            ",d.vat_amount AS ref_vat_amount,d.vat_amount_based AS ref_vat_amount_based " &
            ",c.offset_type,d.offset_type AS ref_offset_type " &
            ",c.general_id,f.general_code,f.general_name,f.address1 AS address,f.tin " &
            ",c.currency_id,h.currency_code,h.currency_name " &
            ",b.project_id,i.project_code,i.project_name " &
            ",b.department_id,j.department_code,j.department_name " &
            "FROM {1}.trans_header a " &
            "LEFT JOIN {1}.trans_jbooks b ON b.trans_id = a.trans_id " &
            "LEFT JOIN {1}.trans_input_vat c ON c.jbooks_id = b.jbooks_id " &
            "LEFT JOIN ( " &
                " SELECT a.offset_type,a.ref_input_vat_id AS input_vat_id,SUM(a.vat_amount) AS vat_amount,SUM(a.vat_amount_based) AS vat_amount_based " &
                " FROM {1}.trans_input_vat a " &
                " GROUP BY a.ref_input_vat_id,a.offset_type " &
            " ) d ON d.input_vat_id = c.input_vat_id " &
            "LEFT JOIN {0}.lib_erp_chart e ON e.account_id = b.account_id " &
            "LEFT JOIN {0}.lib_erp_general f ON f.general_id = c.general_id " &
            "LEFT JOIN {0}.lib_acc_general_multiple g ON g.general_id = f.general_id " &
            "LEFT JOIN {0}.lib_erp_currency h ON h.currency_id = c.currency_id " &
        ...
        'For importation JM
        If input_vat_importation.Checked = True Then
            Try
                sql &= String.Format(" AND c.for_importation = 1 ")
        ...
        If input_vat_importation.Checked = True Then
            ...
            str_name = "INPUT VAT FOR IMPORTATION"
        Else
            str_name = "INPUT VAT"
        ...
        Select Case vat
            Case "1"
                str_title = "Listing of Input VAT"
            Case "2"
                str_title = "Listing of Remitted Input VAT"
                str_columnname = "Amount of Vat. Remitted"
            Case "3"
                str_title = "Listing of Unremitted Input VAT"
            ...
            Case "4"
                str_title = "Listing of Adjusted Input VAT"
        ...
        Else
            ...
            If vat = "5" Then
                templateName = "purchase_vat.xlt"
                excelPurchaseVat(templateName)
        ...
        End If
    End Sub
    Private Sub excelPurchaseVat(ByVal templateName As String)
        Dim date_from As String = txt_date_from.Value.ToString("yyyy-MM-dd")
        Dim date_to As String = txt_date_to.Value.ToString("yyyy-MM-dd")
        Dim vat As String = getRdoValue(vat_filter)
        Dim tDate As Boolean = toDate.Checked
...
" ,b.jbooks_id " & _
" ,c.input_vat_id AS vat_id,c.gross_amount,c.vat_amount,c.net_amount,c.gross_amount_based,c.vat_amount_based,c.net_amount_based,c.vat_class " & _
...
" LEFT JOIN {0}.lib_erp_department j ON j.department_id = b.department_id ", _serverDBFS, _serverDBMain)
        ...
        For Each row As DataRow In dt.Rows
            ...
            xlWorkSheet.Cells(current_row, 10) = 0
            xlWorkSheet.Cells(current_row, 11) = 0
            xlWorkSheet.Cells(current_row, 12) = 0
            Select Case row.Item("vat_class")
                Case "1"
                    xlWorkSheet.Cells(current_row, 10) = row.Item("net_amount")
                Case "2"
                    xlWorkSheet.Cells(current_row, 11) = row.Item("net_amount")
                Case "3"
                    xlWorkSheet.Cells(current_row, 12) = row.Item("net_amount")
            End Select
    ...
    Private Sub excelinputoutput(ByVal listAccount As List(Of Object), ByVal strCol As String, ByVal dicHeader As Dictionary(Of String, String), ByVal templateName As String, ByVal str_title As String, ByVal str_daterange As String, ByVal str_columnname As String)
        ...
        xlWorkSheet.Cells(2, 1) = str_name
    ...
    Private Sub rdo_vat_5_CheckedChanged(sender As Object, e As EventArgs) Handles rdo_vat_5.CheckedChanged
        ...
        End If
    End Sub
    Private Sub inputvat_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        txt_date_from.Value = New Date(Today.Year, 1, 1)
        txt_date_to.Value = Today
    End Sub

    Private Sub Rdo_button2_CheckedChanged(sender As Object, e As EventArgs) Handles input_vat_importation.CheckedChanged
        report_filter()
    End Sub
    Private Sub report_filter()
        Try
            If input_vat.Checked = True Then
                input_vat_importation.Checked = False
            Else
                input_vat.Checked = False
            End If
        Catch ex As Exception
            MsgBox(ex.Message.ToString)
        End Try
    ...
    Private Sub Rdo_button1_CheckedChanged(sender As Object, e As EventArgs) Handles input_vat.CheckedChanged
        report_filter()
    End Sub
End Class