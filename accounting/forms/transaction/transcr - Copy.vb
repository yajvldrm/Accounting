...
Public Class transcr
    ...
    Private Sub txtgenlib_Leave(sender As Object, e As EventArgs) Handles txtgeneral.Leave
        general = FsQuery("select * from lib_erp_general")
        ...
        Else
            ...
            If general.Rows.Count = 1 Then
                dr = general.Rows(0)
            Else
                Dim dra() As DataRow = general.Select("general_code like '" & txtgeneral.Text & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = general
    ...
    Private Sub txtcurrency_Leave(sender As Object, e As EventArgs) Handles txtcurrency.Leave
        ...
        Else
            ...
            If currency.Rows.Count = 1 Then
                dr = currency.Rows(0)
            Else
                Dim dra() As DataRow = currency.Select("currency_name like '" & txtcurrency.Text & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = currency
    ...
    Private Sub txtchart_Leave(sender As Object, e As EventArgs) Handles txtaccount.Leave
        ...
        Else
            ...
            If chart.Rows.Count = 1 Then
                dr = chart.Rows(0)
            Else
                Dim dra() As DataRow = chart.Select("account_code like '" & txtaccount.Text & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = chart
        ...
        If "2310".IndexOf(v.ToString) < 0 Then
            ...
            Else
                ...
                If general.Rows.Count = 1 Then
                    dr = general.Rows(0)
                Else
                    Dim dra() As DataRow = general.Select("general_code like '" & txtgeneraldet.Text & "'")
                    If dra.Count > 0 Then
                        dr = dra(0)
                    Else
                        browse.dt = general
                ...
                Select Case v
                    Case 1
                        ...
                        If dnref Then
                            ...
                            cashap_ds.Clear()
                            dtCopy(cashap_ds, tmp_dt, {"check_id"})
                            cashor.jbooks_id = txtjbooksidtmp.Text
                            cashor.genref_id = txtgeneral.Value
                            cashor.genref_code = txtgeneral.Text
                            cashor.genref_name = txtgeneralname.Text
                            cashor.currency_id = txt_currency_id.Value
                            cashor.exchange_rate = txt_exchange_rate.DecimalValue
                            cashor.based_rate = txt_based_rate.DecimalValue
                            ...
                            If data_trans_jbooks.Rows.Count = 0 Then
                                sql_trans_jbooks = String.Format("SELECT trans_no FROM trans_header WHERE trans_id = {0}", If(txttransid.Text = "", 0, txttransid.Text))
                                data_trans_jbooks = Query(sql_trans_jbooks)
                                If data_trans_jbooks.Rows.Count > 0 Then
                                    cashor.reference_no = data_trans_jbooks.Rows(0).Item("trans_no").ToString
                                End If
                            Else
                                cashor.book_code = data_trans_jbooks.Rows(0).Item("ref_no").ToString
                                cashor.reference_no = data_trans_jbooks.Rows(0).Item("trans_no").ToString
                            End If
                            ...
                            If ds.Tables("checks").Select("jbooks_id = '" & txtjbooksidtmp.Text & "' AND deleted = 'False'").Length > 0 Then
                                debit = ds.Tables("checks").Compute("Sum(check_amount)", "jbooks_id = '" & txtjbooksidtmp.Text & "' AND deleted = 'False'")
                            End If
                        End If
                    Case 3
                        If dnref Then
                            ...
                            offsetap.department_id = txtdebit.Value
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        If v.ToString = "1" Then
            If ds.Tables("jbooks").Select("validation = '1' AND deleted = 'False'").Count > 0 Then
                txtamount.DecimalValue = ds.Tables("jbooks").Compute("sum(debit)", "validation = '1' AND deleted = 'False'")
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If ds.Tables("checks").Rows.Count > 0 Then
            For i = 0 To ds.Tables("checks").Rows.Count - 1
                ...
                fields.Add("bank_code", dr("bank_code"))
                fields.Add("bank_name", dr("bank_name"))
    ...
    Friend Sub refresh_record()
        ...
        '    ",ifnull(b.si_no,'') as si_no,ifnull(b.trans_cr_id,'') AS trans_cr_id " & _
        '    ",c.general_code,c.general_name " & _
        '    ",d.currency_code,d.currency_name " & _
        '    "from trans_header a " & _
        '    "left join trans_header_cr b on a.trans_id = b.trans_id " & _
        ...
        Dim sql As String = String.Format("SELECT currentapprover.user_id AS current_approver,currentapprover.status AS current_status " & _
            ...
            ",c.general_code,c.general_name " & _
            ",d.currency_code,d.currency_name " & _
            ",IFNULL(b.trans_cr_id,'') AS trans_cr_id,IFNULL(b.si_no,'') AS si_no " & _
            "FROM trans_header a " & _
            "LEFT JOIN trans_header_cr b ON a.trans_id = b.trans_id " & _
    ...
    Private Sub bind_control()
        ...
        txttranscrid.Text = dr("trans_cr_id")
    ...
    Private Sub txtproject_Leave(sender As Object, e As EventArgs) Handles txtproject.Leave
        ...
        Else
            ...
            If project.Rows.Count = 1 Then
                dr = project.Rows(0)
            Else
                Dim dra() As DataRow = project.Select("project_name like '" & txtproject.Text & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = project
    ...
    Private Sub txtdepartment_Leave(sender As Object, e As EventArgs) Handles txtdepartment.Leave
        ...
        Else
            ...
            Else
                Dim dra() As DataRow = department.Select("department_name like '" & txtdepartment.Text & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
    ...
    Private Sub txtallocation_Leave(sender As Object, e As EventArgs) Handles txtallocation.Leave
        ...
        Else
            ...
            Else
                Dim dra() As DataRow = allocation.Select("allocation_name like '" & txtallocation.Text & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
    ...
    Private Sub txt_currency_id_Leave(sender As Object, e As EventArgs) Handles txt_currency_id.Leave
        ...
        Else
            ...
            If dt.Rows.Count = 1 Then
                dr = dt.Rows(0)
            Else
                Dim dra() As DataRow = dt.Select(String.Format("currency_name like '{0}'", txt_currency_id.Text))
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = dt
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        report_cr_filter.journal_id = journal_id
        report_cr_filter.book_id = book_id
        report_cr_filter.ShowDialog()
    End Sub
    ...
    Private Sub txttermsiddet_Leave(sender As Object, e As EventArgs) Handles txttermsiddet.Leave
        ...
        Else
            ...
            If terms.Rows.Count = 1 Then
                dr = terms.Rows(0)
            Else
                Dim dra() As DataRow = terms.Select("terms_name like '" & txttermsiddet.Text & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = terms
    ...
    Private Sub btnSettings_Click(sender As Object, e As EventArgs) Handles btnSettings.Click
        Dim dr As DataRow
        dr = main.Journal.Select("journal_code = 'CR'")(0)
