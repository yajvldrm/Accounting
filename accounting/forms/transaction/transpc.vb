...
Public Class transpc
    ...
    Private module_code As String = "ACC00601"
    ...
    Private Sub enable_control()
        ...
        Dim dr As DataRow = main.Sys_Default_Det.Select("module_code = 'ACC00601'")(0)
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        txtday.Text = Now().Day
        txtreflenishdate.Text = String.Format("{0:d}", DateTime.Today)
        txtduedate.Text = String.Format("{0:d}", DateTime.Today)
    ...
    Private Sub txtcurrency_Leave(sender As Object, e As EventArgs) Handles txtcurrency.Leave
        ...
        Else
            ...
            If currency.Rows.Count = 1 Then
                dr = currency.Rows(0)
            Else
                Dim dra() As DataRow = currency.Select("currency_name like '" & txtcurrency.Text.ToString.Replace("'", "''") & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = currency
    ...
    Private Sub txtcredit_Leave(sender As Object, e As EventArgs) Handles txtcredit.Leave
        ...
        End If
        If v <> 2 Then
            txtcreditbased.DecimalValue = txtcredit.DecimalValue * (txt_exchange_rate.DecimalValue / txt_based_rate.DecimalValue)
            txtdnref.Enabled = False
        Else
            txtdnref.Enabled = True
            txtdnref.Focus()
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        'fields.Add("reflenish_date", txtreflenishdate.Value.ToString("yyyy-MM-dd"))
        ...
        fields.Add("trans_pc_id", txttranspcid.Text)
        fields.Add("reflenish_date", String.Format("{0:yyyy-MM-dd}", txtreflenishdate.Text))
        fields.Add("due_date", String.Format("{0:yyyy-MM-dd}", txtduedate.Text))
        ...
        If txttranspcid.Text = "" Then
            qry = genInsert("trans_header_pc", fields)
        Else
            qry = genUpdate("trans_header_pc", fields)
        End If
        dt = Query(qry)
    ...
    Friend Sub refresh_record()
        Dim sql As String = String.Format("SELECT currentapprover.user_id AS current_approver,currentapprover.status AS current_status " & _
            ...
            ",a.* " & _
            ",ifnull(b.trans_pc_id,0) AS trans_pc_id,b.reflenish_date,ifnull(b.po_no,'') AS po_no,ifnull(b.dr_no,'') AS dr_no,ifnull(b.si_no,'') AS si_no,ifnull(b.terms_id,0) AS terms_id,ifnull(b.nodays,0) AS nodays,b.due_date,e.terms_name " & _
            ",c.general_code,c.general_name " & _
            ",d.currency_code,d.currency_name " & _
            "from trans_header a " & _
            "left join trans_header_pc b on a.trans_id = b.trans_id " & _
    ...
    Private Sub bind_control()
        ...
        txttranspcid.Text = dr("trans_pc_id")
        txttransno.Text = dr("trans_no")
        txtday.Text = Format(dr("trans_date"), "dd")
        txtreflenishdate.Text = String.Format("{0:d}", dr("reflenish_date"))
        txtgeneral.Value = dr("general_id")
        txtgeneral.Text = dr("general_code")
        txtgeneralname.Text = dr("general_name")
        txtcurrency.Value = dr("currency_id")
    ...
    Private Sub txt_currency_id_Leave(sender As Object, e As EventArgs) Handles txt_currency_id.Leave
        ...
        Else
            ...
            If dt.Rows.Count = 1 Then
                dr = dt.Rows(0)
            Else
                Dim dra() As DataRow = dt.Select(String.Format("currency_code like '{0}'", txt_currency_id.Text.ToString.Replace("'", "''")))
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = dt
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        report_pc_filter_new.journal_id = journal_id
        report_pc_filter_new.book_id = book_id
        report_pc_filter_new.ShowDialog()
    End Sub
    ...
    Private Sub txtterms_Leave(sender As Object, e As EventArgs) Handles txtterms.Leave
        ...
        Else
            ...
            If terms.Rows.Count = 1 Then
                dr = terms.Rows(0)
            Else
                Dim dra() As DataRow = terms.Select("terms_name like '" & txtterms.Text.ToString.Replace("'", "''") & "'")
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = terms
    ...
    Private Sub btnSettings_Click(sender As Object, e As EventArgs) Handles btnSettings.Click
        Dim dr As DataRow
        dr = main.Journal.Select("journal_code = 'PC'")(0)
