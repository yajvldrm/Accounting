...
Public Class offsetinputvat_importation
    ...
    Public Sub refresh_data()
        ...
        If trans_id = "" Then
            sql = String.Format("SELECT a.general_id,a.currency_id,a.exchange_rate,a.based_rate,a.jbooks_id,a.input_vat_id AS ref_input_vat_id,(a.vat_amount - IFNULL(f.vat_amount,0)) AS tmp_amount " &
                ...
                "AND a.offset_type = 0 AND (a.vat_amount - IFNULL(f.vat_amount,0)) <> 0  and a.for_importation = '1' ", account_id, filter_date, general_id, currency_id, If(project_id = "", " IS NULL ", " = " & project_id), If(department_id = "", " IS NULL ", " = " & department_id), _serverDBFS, If(BookID = "", "IS NULL", "= " & BookID))
        Else
            sql = String.Format("SELECT a.general_id,a.currency_id,a.exchange_rate,a.based_rate,a.jbooks_id,a.input_vat_id AS ref_input_vat_id,(a.vat_amount - IFNULL(f.vat_amount,0)) AS tmp_amount " &
                ",IFNULL(g.journal_code,'') AS ref_no,d.trans_no,d.trans_date,e.general_code,e.general_name,b.account_id,0 AS vat_amount
                ,d.description,b.line_remarks AS particulars " &
                ...
                "AND (a.offset_type = 0 OR a.offset_type IS NULL) AND (a.vat_amount - IFNULL(f.vat_amount,0)) <> 0  and a.for_importation = 1 ", account_id, trans_id, filter_date, general_id, currency_id, If(project_id = "", " IS NULL ", " = " & project_id), If(department_id = "", " IS NULL ", " = " & department_id), _serverDBFS, If(BookID = "", "IS NULL", "= " & BookID))
        End If
        ...
        For Each row As DataRow In _dt.Rows
            row.Item("jbooks_id") = jbooks_id
            ds.Tables("input_vat_for_importation").ImportRow(row)
        Next
    End Sub
    ...
    Private Sub cboAll_CheckedChanged(sender As Object, e As EventArgs) Handles cboAll.CheckedChanged
        ...
        Next
        For Each dr As DataRow In ds.Tables("input_vat_for_importation").Rows
    ...
    Private Sub dgOffset_CellFormatting_1(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles dgOffset.CellFormatting
        Dim drv As DataRowView
        If e.RowIndex >= 0 Then
            If e.RowIndex <= ds.Tables("input_vat_for_importation").Rows.Count - 1 Then
                drv = ds.Tables("input_vat_for_importation").DefaultView.Item(e.RowIndex)
                Dim c As Color
    ...
    Private Sub dgOffset_CellValueChanged_1(sender As Object, e As DataGridViewCellEventArgs) Handles dgOffset.CellValueChanged
    ...
    Private Sub dgOffset_CurrentCellDirtyStateChanged_1(sender As Object, e As EventArgs) Handles dgOffset.CurrentCellDirtyStateChanged
    ...
    Private Sub dgOffset_DataBindingComplete_1(sender As Object, e As DataGridViewBindingCompleteEventArgs) Handles dgOffset.DataBindingComplete
        Dim dt As DataTable = ds.Tables("input_vat_for_importation")
        If dt.Rows.Count > 0 Then
    ...
    Private Sub dgOffset_DataError_1(sender As Object, e As DataGridViewDataErrorEventArgs) Handles dgOffset.DataError
        Dim ex = e.Exception
