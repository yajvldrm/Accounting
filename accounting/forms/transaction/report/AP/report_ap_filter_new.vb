...
Public Class report_ap_filter_new
    'raymond - 2016-06-10
    ...
    Private Sub report_ap_filter_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        txt_print._Text = "Purchase Voucher"
        txt_print_Leave(Nothing, Nothing)
        invoice_from.Text = trans_no
        invoice_to.Text = trans_no
    ...
    Private Sub txt_print_Leave(sender As Object, e As EventArgs) Handles txt_print.Leave
        Dim dt As DataTable = FsQuery("SELECT 'Purchase Voucher' AS name,'' AS code,1 AS id UNION SELECT 'Purchase Voucher List','',2")
        ...
        Else
            ...
            If dt.Rows.Count = 1 Then
                dr = dt.Rows(0)
            Else
                Dim dra() As DataRow = dt.Select(String.Format("name like '{0}'", txt_print._Text.ToString.Replace("'", "''")))
                If dra.Count > 0 Then
                    dr = dra(0)
                Else
                    browse.dt = dt
    ...
    Private Sub invoice_Leave(sender As Object, e As EventArgs) Handles invoice_from.Leave, invoice_to.Leave
        Dim sql As String = String.Format("SELECT a.trans_id,a.trans_no,a.trans_date " & vbNewLine &
            "FROM trans_header a " & vbNewLine &
            "LEFT JOIN {1}.lib_erp_book b ON b.book_id = a.book_id " & vbNewLine &
            "WHERE a.journal_id = '{0}' AND b.company_id = '{2}' " & vbNewLine &
            "ORDER BY a.trans_date DESC,a.trans_no DESC " & vbNewLine &
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        sql_header = String.Format("SELECT a.trans_id,a.trans_no,a.trans_date,b.currency_name,b.currency_code,b.symbol,a.amount,a.amount_based,a.exchange_rate,a.based_rate,a.description,a.general_id,c.general_code,c.general_name,c.address1 AS address,c.attention,IF(a.status = 'A','Approved',(IF(a.status = 'C','Cancelled',''))) AS status " &
                                   ",d.due_date,d.si_no,d.rr_no,d.po_no,d.terms_id,d.nodays,e.terms_name 
                                   ,e.user_id,e.username,e.user_fullname 
                                   ,_book.book_code,_book.book_name " &
                                       ...
                                       "LEFT JOIN {2}.trans_header_ap d ON d.trans_id = a.trans_id " &
                                       ...
                                       "LEFT JOIN (
                                            ...
                                            GROUP BY a.trans_id
                                       ) e ON e.trans_id = a.trans_id " &
                                       "WHERE a.journal_id = {0} ", journal_id, _serverDBFS, _serverDBMain)
        '   jbooks
        sql_jbooks = String.Format("SELECT a.trans_id,a.jbooks_id,b.account_code,b.account_name,a.debit,a.credit,a.debit_based,a.credit_based,c.general_code,c.general_name,i.trans_no AS ref_ref_no,f.project_code,f.project_name " &
        ...
        sql_jbooks &= " ,IF(a.debit <> 0,1,2),b.account_code "
        sql_particular = String.Format("SELECT a.trans_particular_id,a.trans_id,a.trans_particular_code,a.trans_particular_name,a.trans_particular_amount " &
        ...
        sql_signatory = String.Format("SELECT a.user_signatory_id,a.user_id,a.label1,a.user_id1 " &
            ...
            "", main.user_id, "ACC00301")

        'Dim colPrepared As New List(Of KeyValuePair(Of String, String))
        'colPrepared.Add(New KeyValuePair(Of String, String)("dbMain", ""))
        'colPrepared.Add(New KeyValuePair(Of String, String)("dbFS", ""))
        'colPrepared.Add(New KeyValuePair(Of String, String)("trans_id", ""))
        'Dim dtPrepared As DataTable = ExecuteWithParameter("getPrepared", ConnectionStringMain, CommandType.StoredProcedure, colPrepared)

        Dim col As New List(Of KeyValuePair(Of String, String))
        col.Add(New KeyValuePair(Of String, String)("@module_code", "ACC00301"))
        ...
        Select Case _print
            Case "1" ' Invoice
                ...
                '   _currency
                createData()
                'ds.Tables("Prepared").Clear()
                'For Each dr As DataRow In dtPrepared.Rows
                '    ds.Tables("Prepared").ImportRow(dr)
                'Next
    ...
    Public Sub generateReport(ByVal rep As XtraReport, ByVal withProject As Boolean, ByVal withDepartment As Boolean, ByVal withAllocation As Boolean, Optional withCompare As Boolean = False)
        ...
        If rep.FindControl("panel_header", False) IsNot Nothing Then
        ...
        cellHeaderCode.TextAlignment = DevExpress.XtraPrinting.TextAlignment.MiddleLeft
        cellHeaderTitle.Text = "ACCOUNT TITLE"
        cellHeaderTitle.WidthF = 200
        cellHeaderTitle.TextAlignment = DevExpress.XtraPrinting.TextAlignment.MiddleLeft
