...
Public Class report_pc_filter
    ...
    Private Sub report_ap_filter_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        txt_print._Text = "Petty Cash Voucher"
    ...
    Private Sub txt_print_Leave(sender As Object, e As EventArgs) Handles txt_print.Leave
        ...
        Dim dt As DataTable = FsQuery("SELECT 'Petty Cash Voucher' AS name,'' AS code,1 AS id UNION SELECT 'Petty Cash Voucher List','',2")
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        Dim _withEntry = withEntry.Checked
        ...
        sql_header = String.Format("SELECT a.trans_id,a.trans_no,a.trans_date,b.currency_name,b.currency_code,a.amount,a.amount_based,a.exchange_rate,a.based_rate,a.description,a.general_id,c.general_code,c.general_name,c.address1 AS address,c.attention,IF(a.status = 'A','Approved',(IF(a.status = 'C','Cancelled',''))) AS status " & _
                                   ",d.due_date,d.si_no,d.dr_no,d.po_no,d.terms_id,d.nodays,e.terms_name " & _
                                       "FROM trans_header a " & _
                                       ...
                                       "LEFT JOIN {2}.trans_header_pc d ON d.trans_id = a.trans_id " & _
        ...
        Select Case _print
            Case "1" ' Invoice
                ...
                If Not _withEntry Then
                    generateDuravilleReport(New valiant_voucher)
                Else
                    If _currency = "1" Or _currency = "2" Then
                        Dim rep As New report_pc
                        rep.DataSource = ds
                        Dim frm As String = ""
            ...
            Case "2" ' Invoice List
                Select Case _invoice
                    Case "1" ' By Invoice No
                        ...
                        Dim rep As New report_pc_print_2
                        rep.DataSource = ds
                    ...
                    Case "3" ' By Client
                        Select Case _client
                            Case "1" ' All Clients
                                ...
                                Dim rep As New report_pc_print_3
                                rep.DataSource = ds
                                rep.Parameters("runtime").Value = System.DateTime.Now.ToString("MM/dd/yyyy - HH:mm:ss")
