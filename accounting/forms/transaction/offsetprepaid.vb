...
Public Class offsetprepaid
        ...
        If dt.Rows.Count > 0 Then
            For Each row As DataRow In dt.Rows
                ...
                Try
                    ...
                    Else
                        If row.Item("ref_prepaid_id") = "" Then
                            dt.Rows.Remove(row)
                        Else
    ...
    Public Sub refresh_data()
        Dim sql = ""
        If trans_id = "" Then
            sql = String.Format("SELECT a.date_acquired,a.date_start,a.jbooks_id,a.currency_id,a.exchange_rate,a.based_rate,a.description " &
                ",IFNULL(f.journal_code,'') AS ref_no,d.trans_no,d.trans_date,a.prepaid_id AS ref_prepaid_id,0 AS amount,(a.amount - IF(e.amount IS NULL,0,e.amount)) AS tmp_amount,b.line_remarks AS particulars " &
                ...
                "LEFT JOIN (SELECT a.ref_prepaid_id AS prepaid_id,SUM(a.amount) AS amount FROM trans_prepaid a GROUP BY a.ref_prepaid_id) e ON e.prepaid_id = a.prepaid_id " &
                "LEFT JOIN {6}.lib_erp_journal f ON f.journal_id = d.journal_id " &
                "WHERE b.account_id = {0} " &
                "AND b.currency_id = {2} " &
                "AND b.project_id {3} " &
                "AND b.department_id {4} " &
                "AND d.trans_date <= '{1}' AND a.ref_prepaid_id IS NULL AND (a.amount - IF(e.amount IS NULL,0,e.amount)) <> 0 ", account_id, filter_date, currency_id, If(project_id = "", " IS NULL ", " = " & project_id), If(department_id = "", " IS NULL ", " = " & department_id), If(BookID = "", "IS NULL", "= " & BookID), _serverDBFS)
        Else
            sql = String.Format("SELECT a.date_acquired,a.date_start,a.jbooks_id,a.currency_id,a.exchange_rate,a.based_rate,a.description " &
                ",IFNULL(f.journal_code,'') AS ref_no,d.trans_no,d.trans_date,a.prepaid_id AS ref_prepaid_id,0 AS amount,(a.amount - IF(e.amount IS NULL,0,e.amount)) AS tmp_amount,b.line_remarks AS particulars " &
                "FROM trans_prepaid a " &
                ...
                "LEFT JOIN trans_jbooks_ref c ON c.jbooks_id = b.jbooks_id " &
                "LEFT JOIN trans_header d ON d.trans_id = b.trans_id " &
                "LEFT JOIN (SELECT a.ref_prepaid_id AS prepaid_id,SUM(a.amount) AS amount FROM trans_prepaid a LEFT JOIN trans_jbooks b ON b.jbooks_id = a.jbooks_id WHERE b.trans_id <> {1} GROUP BY a.ref_prepaid_id) e ON e.prepaid_id = a.prepaid_id " &
                ...
                "AND d.trans_id <> {1} " &
                "AND d.trans_date <= '{2}' AND a.ref_prepaid_id IS NULL AND (a.amount - IF(e.amount IS NULL,0,e.amount)) <> 0 ", account_id, trans_id, filter_date, currency_id, If(project_id = "", " IS null ", " = " & project_id), If(department_id = "", " IS NULL ", " = " & department_id), If(BookID = "", "IS NULL", "= " & BookID), _serverDBFS)
        End If
        ...
        For Each row As DataRow In _dt.Rows
            row.Item("jbooks_id") = jbooks_id
            ds.Tables("prepaid").ImportRow(row)
        Next
    End Sub
    Private Sub offsetprepaid_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        txtDateFrom.Value = CDate(main.Sys_Default.Rows(0).Item("sys_beg").ToString())
        dgOffset.AutoGenerateColumns = False
        dgOffset.DataSource = ds.Tables("prepaid")
    ...
    Private Sub dgOffset_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgOffset.CellValueChanged
        ...
        Dim drv As DataRowView = bs_prepaid.Current
    ...
    Private Sub cboALl_CheckedChanged(sender As Object, e As EventArgs) Handles cboALl.CheckedChanged
        ...
        For Each row As DataGridViewRow In dgOffset.Rows
            ...
            dr.Item("credit") = dr.Item("amount")
            dr.Item("credit_based") = dr.Item("amount") * (dr.Item("exchange_rate") / dr.Item("based_rate"))
            dr.Item("amount_based") = dr.Item("amount") * (dr.Item("exchange_rate") / dr.Item("based_rate"))
            dr.Item("balance") = dr.Item("tmp_amount") - dr.Item("amount")
