...
Public Class offsetap
    ...
    Friend Sub initializeDecimalPlaces(ByVal strNumberFormat As String)
        ...
        dgOffset.Columns("debit").DefaultCellStyle.Format = strNumberFormat
    ...
    Public Sub refresh_data()
        ...
        If trans_id = "" Then
            sql = String.Format("SELECT a.jbooks_id AS ref_jbooks_id,'' AS jbooks_id_tmp,'' AS trans_id,IFNULL(m.journal_code,'') AS ref_journal_code " &
                ...
                ",a.account_id,g.account_code,g.account_name,g.validation,g.noCompute " &
                ...
                ,g.unit_applicable,a.book_unit_no_id,o.book_unit_no_code,o.book_unit_no_name " &
                ",c.description,a.line_remarks AS particulars " & vbNewLine &
                "FROM trans_jbooks a " &
                ...
                "LEFT JOIN {7}.trans_jbooks_ap k ON k.jbooks_id = a.jbooks_id " &
                "LEFT JOIN {6}.lib_erp_terms l ON l.terms_id = k.terms_id " &
                "LEFT JOIN {6}.lib_erp_journal m ON m.journal_id = c.journal_id " &
                "LEFT JOIN {7}.trans_header_ap n ON n.trans_id = c.trans_id " &
                ...
                "AND a.ref_jbooks_id IS NULL AND (a.credit - IF(j.debit IS NULL,0,j.debit)) <> 0 ", account_id, filter_date, general_id, currency_id, If(project_id = "", " IS NULL ", " = " & project_id), If(department_id = "", " IS NULL", " = " & department_id), _serverDBFS, _serverDBMain, If(BookID = "", "IS NULL", "= " & BookID), book_unit_no_id)
        Else
            sql = String.Format("SELECT a.jbooks_id AS ref_jbooks_id,'' AS jbooks_id_tmp,IFNULL(m.journal_code,'') AS ref_journal_code " &
                ...
                "LEFT JOIN {8}.trans_jbooks_ap k ON k.jbooks_id = a.jbooks_id " &
                "LEFT JOIN {7}.lib_erp_terms l ON l.terms_id = k.terms_id " &
                "LEFT JOIN {7}.lib_erp_journal m ON m.journal_id = c.journal_id " &
                "LEFT JOIN {8}.trans_header_ap n ON n.trans_id = c.trans_id " &
                ...
                "AND a.ref_jbooks_id IS NULL AND (a.credit - IF(j.debit IS NULL,0,j.debit)) <> 0 ", account_id, filter_date, trans_id, general_id, currency_id, If(project_id = "", " IS NULL ", " = " & project_id), If(department_id = "", " IS NULL ", " = " & department_id), _serverDBFS, _serverDBMain, If(BookID = "", "IS NULL", "= " & BookID), book_unit_no_id)
        End If
    ...
    Private Sub offsetjbooks_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'OriginalDT = ds.Tables("jbooks").Clone()
    ...
    Private Sub dgOffset_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgOffset.CellValueChanged
        ...
        If drv IsNot Nothing Then
            ...
            If dr.Item("isInputted") Then
                If dr.Item("debit") = 0 Then
                    dr.Item("debit") = dr.Item("tmp_amount")
                End If
            Else
                dr.Item("debit") = 0.0
            End If
            ...
            dr.Item("balance") = dr.Item("tmp_amount") - dr.Item("debit")
    ...
    Private Sub dgOffset_CurrentCellDirtyStateChanged(sender As Object, e As EventArgs) Handles dgOffset.CurrentCellDirtyStateChanged
        'If dgOffset.CurrentCell.ColumnIndex = 9 Then
        '    If dgOffset.IsCurrentCellDirty Then
        '        dgOffset.CommitEdit(DataGridViewDataErrorContexts.Commit)
        '    End If
        'End If
        '    If (dgvStandingOrder.CurrentCell Is DataGridViewCheckBoxCell) Then
        '            {
    ...
    Private Sub dgOffset_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgOffset.CellContentClick
        ...
        If colName = "showSubConOffset" Then
            ...
            For Each dr As DataRow In ds.Tables("trans_sub_contract_offset").Rows
                Dim id As String = ifNull(dr("offset_sub_contract_id"), "").ToString()
            ...
            Dim SCAP As New offsetSubCon
            SCAP.TransID = trans_id
            SCAP.OffsetID = String.Join(",", lstOffset)
            SCAP.MyParent = Me
            SCAP.ShowDialog()
            If SCAP.DialogResult = DialogResult.OK Then
                'ds.Tables("project_management_offset").Clear()
                For Each dr As DataRow In SCAP.ds.Tables("trans_sub_contract_offset").Rows
                    Dim drs = ds.Tables("trans_sub_contract_offset").Select(String.Format("trans_sub_contract_id = '{0}'", dr("trans_sub_contract_id")))
                    If drs.Length > 0 Then
                        drs(0)("sel") = dr("sel")
