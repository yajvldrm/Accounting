...
Public Class begbal_ov_detail
    Dim dbTable As String = "trans_output_vat"
    ...
    Private Sub init_load()
        ...
        If jbooks_id <> "0" And jbooks_id <> "" Then
            Dim sql = String.Format("SELECT a.output_vat_id,a.jbooks_id,a.general_id,a.currency_id,a.exchange_rate " &
                ",a.based_rate,a.offset_type,a.vat_type,a.vat_rate,a.gross_amount,a.vat_amount,a.net_amount " &
                ",a.debit,a.credit,a.gross_amount_based,a.vat_amount_based,a.net_amount_based " &
                ",a.debit_based,a.credit_based,a.ref_output_vat_id,b.general_name,b.tin " &
                ",CONCAT(b.address1,',',b.address2,',',b.address3,',',b.address4) AS address " &
                ",c.currency_name,d.output_vat_ref_id,d.ref_no " &
                ...
                "LEFT JOIN {2}.trans_output_vat_ref d ON d.output_vat_id = a.output_vat_id " &
                "LEFT JOIN (" &
                    "SELECT c.ref_output_vat_id AS output_vat_id,SUM(c.debit) AS debit,SUM(c.credit) AS credit " &
                    ",SUM(c.debit_based) AS debit_based,SUM(c.credit_based) AS credit_based " &
                    "FROM trans_header a " &
                    "LEFT JOIN trans_jbooks b ON b.trans_id = a.trans_id " &
                    "LEFT JOIN trans_output_vat c ON c.jbooks_id = b.jbooks_id " &
                    "WHERE c.ref_output_vat_id IS NOT NULL AND (a.status <> 'C' OR a.status IS NULL) " &
                    "GROUP BY c.ref_output_vat_id " &
                ") e ON e.output_vat_id = a.output_vat_id " &
                "WHERE a.jbooks_id = {0}", jbooks_id, _serverDBFS, _serverDBMain)
        ...
        End If
        Dim tax_dt As DataTable = FsQuery(String.Format("SELECT * FROM lib_acc_output_vat WHERE effectivity_date <= '{0}' ORDER BY effectivity_date DESC LIMIT 1", System.DateTime.Today.ToString("s")))
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Dim output_vat_id = txt_output_vat_id.Value
        Dim j_books_id = jbooks_id
        Dim gen_lib_id = txt_general_id.Value
        ...
        dr.Item("output_vat_id") = output_vat_id
        dr.Item("jbooks_id") = j_books_id
        dr.Item("ref_no") = ref_no
        dr.Item("general_id") = gen_lib_id
        ...
        dr.Item("credit_based") = credit * (exchange_rate / based_rate)
        ...
        Else
            If transmode = "insert" Then
                If DataExist(dgMain, {"output_vat_id", txt_output_vat_id.Value}, {"ref_no"}, {txt_ref_no.Text}) Then
                    MsgBox("Output VAT already exist")
                    err = True
                Else
                    dr.Item("output_vat_id") = "ag-" & dt.Rows.Count
                    dt.Rows.Add(dr)
                    pk = dr.Item("output_vat_id")
                End If
            ElseIf transmode = "update" Then
                If DataExist(dgMain, {"output_vat_id", txt_output_vat_id.Value}, {"ref_no"}, {txt_ref_no.Text}, False) Then
                    MsgBox("Output VAT already exist")
                    err = True
                Else
                    Dim id = dgMain.SelectedRows(0).Cells("output_vat_id").Value.ToString
                    Dim row As DataRow = dt.Select(String.Format("output_vat_id = '{0}'", id))(0)
                    row.Item("ref_no") = ref_no
                    row.Item("general_id") = gen_lib_id
                    ...
                    row.Item("credit_based") = credit * (exchange_rate / based_rate)
    ...
    Private Sub txt_genref_Leave(sender As Object, e As EventArgs) Handles txt_general_id.Leave
        Dim dt As DataTable = FsQuery("SELECT *,CONCAT(address1,',',address2,',',address3,',',address4) AS address,general_id FROM lib_erp_general ORDER BY FIELD(category_id,1,2),general_code ASC ")
    ...
    Private Sub dgMain_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles dgMain.CellFormatting
        For Each row As DataGridViewRow In dgMain.Rows
            If row.Cells("status").Value <> "" Then
    ...
    Private Sub btnRemove_Click(sender As Object, e As EventArgs) Handles btnRemove.Click
        Dim selectedIDS = getCheckedGrid("output_vat_id", "ref_no", "sel", dgMain, , True, , True)
        If selectedIDS.Length > 0 Then
            For Each id As String In selectedIDS
                Dim selectedRows = ds.Tables(dbTable).Select(String.Format("output_vat_id = '{0}'", id))
                If selectedRows.Count > 0 Then
                    Dim selectedRow = selectedRows(0)
    ...
    Private Sub btnViewDetails_Click(sender As Object, e As EventArgs) Handles btnViewDetails.Click
        Dim id As String = txt_output_vat_id.Value.ToString
        If id <> "" Then
            Try
                ...
                Dim sql = String.Format("SELECT a.trans_no AS ref_no,a.trans_date,d.book_code,e.account_code,e.account_name,b.debit,b.credit " & _
                    ...
                    "LEFT JOIN trans_output_vat c ON c.jbooks_id = b.jbooks_id " & _
                    "LEFT JOIN {1}.lib_erp_book d ON d.book_id = a.book_id " & _
                    "LEFT JOIN {1}.lib_erp_chart e ON e.account_id = b.account_id " & _
                    "WHERE c.ref_output_vat_id = '{0}' AND (a.status <> 'C' OR a.status IS NULL)", id, _serverDBFS)
    ...
    Private Sub begbal_ov_detail_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        ...
        End If
        If ds.Tables("trans_output_vat").Rows.Count = 0 Then
            ds.Tables("trans_output_vat").Clear()
            Dim dt_iv As DataTable = dgMain.DataSource
            For Each dr As DataRow In dt_iv.Rows
                ds.Tables("trans_output_vat").ImportRow(dr)
            Next
        End If
        If ds.Tables("trans_output_vat").Rows.Count > 0 Then
            If My.Settings._enableUnitNo Then
                For Each dr As DataRow In ds.Tables("trans_output_vat").Rows
                    If Not lstUnitID.ContainsKey(dr("unit_id")) Then
                        ...
                        If drs.Length > 0 Then
                            ...
                            If jbooks_id.Contains("tmp_") Or jbooks_id = "" Then
                                jbooks_credit = ds.Tables("trans_output_vat").Compute("SUM(credit)", String.Format("unit_id = '{0}' AND deleted = False", dr("unit_id")))
                                jbooks_credit_based = ds.Tables("trans_output_vat").Compute("SUM(credit_based)", String.Format("unit_id = '{0}' AND deleted = False", dr("unit_id")))
                ...
                For Each dr As DataRow In ds.Tables("trans_output_vat").Rows
                    ...
                    Dim output_vat_id = dr("output_vat_id")
                    Dim output_vat_ref_id = dr("output_vat_ref_id")
                    ...
                    End If
                    Dim dic As New Dictionary(Of String, String)
                    dic.Add("output_vat_id", output_vat_id)
                    dic.Add("jbooks_id", jbooks_id)
                    ...
                    dic.Add("gross_amount_based", gross_amount_based)
                    dic.Add("net_amount_based", net_amount_based)
                    dic.Add("debit", 0)
                    dic.Add("debit_based", 0)
                    If output_vat_id.ToString.Contains("ag-") Or output_vat_id.ToString.Contains("tmp_") Or output_vat_id = "" Then
                        If Not deleted Then
                            Dim sql As String = ATGenerateQuery("Insert", "trans_output_vat", dic, Environment.MachineName, Environment.UserName, main.log_id, "|", "output_vat_id")
                            Dim return_id = Query(sql)
                            output_vat_id = return_id.Rows(0)("return_id")
                            Dim dicDet As New Dictionary(Of String, String)
                            dicDet.Add("output_vat_ref_id", "")
                            dicDet.Add("output_vat_id", output_vat_id)
                            dicDet.Add("ref_no", ref_no)
                            Dim sql_ref As String = ATGenerateQuery("Insert", "trans_output_vat_ref", dicDet, Environment.MachineName, Environment.UserName, main.log_id, "|", "output_vat_ref_id")
                            Query(sql_ref)
                        End If
                    Else
                        If deleted Then
                            Dim sql As String = ATGenerateQuery("DELETE", "trans_output_vat", Nothing, Environment.MachineName, Environment.UserName, main.log_id, output_vat_id & "|", "output_vat_id")
                            Query(sql)
                        Else
                            Dim sql As String = ATGenerateQuery("UPDATE", "trans_output_vat", dic, Environment.MachineName, Environment.UserName, main.log_id, output_vat_id & "|", "output_vat_id")
                            Query(sql)
                            Dim return_id = Query(sql)
                            output_vat_id = return_id.Rows(0)("return_id")
                            Dim dicDet As New Dictionary(Of String, String)
                            dicDet.Add("output_vat_ref_id", output_vat_ref_id)
                            dicDet.Add("output_vat_id", output_vat_id)
                            dicDet.Add("ref_no", ref_no)
                            Dim sql_ref As String = ATGenerateQuery("Update", "trans_output_vat_ref", dicDet, Environment.MachineName, Environment.UserName, main.log_id, output_vat_ref_id & "|", "output_vat_ref_id")
                            Query(sql_ref)
                        End If
                    End If
                Next
            Else
                ...
                If drs.Length > 0 Then
                    jbooks_id = drs(0)("jbooks_id")
                    If jbooks_id.Contains("tmp_") Or jbooks_id = "" Then
                        jbooks_credit = ds.Tables("trans_output_vat").Compute("SUM(credit)", String.Format("deleted = False"))
                        jbooks_credit_based = ds.Tables("trans_output_vat").Compute("SUM(credit_based)", String.Format("deleted = False"))
                        If jbooks_credit <> 0 Or jbooks_credit_based <> 0 Then
                            Dim dic As New Dictionary(Of String, String)
                            dic.Add("jbooks_id", "")
                            dic.Add("trans_id", trans_id)
