...
Public Class begbal_ar_detail
    ...
    Private Sub init_load()
        ...
        If trans_id <> "" And account_id <> "" Then
            Dim sql = String.Format("SELECT a.jbooks_id,a.trans_id,a.project_id,a.department_id,a.allocation_id " &
                ...
                ",f.terms_id,IFNULL(f.nodays,0) AS nodays,f.dr_no,f.si_no,f.po_no " &
                ...
                "LEFT JOIN {3}.trans_jbooks_ar f ON f.jbooks_id = a.jbooks_id " &
                ...
                "WHERE c.validation = 2 AND a.trans_id = {0} AND a.account_id = {1}
                AND IFNULL(a.project_id,'') = '{4}' AND IFNULL(a.department_id,'') = '{5}'", trans_id, account_id, _serverDBFS, _serverDBMain, project_id, department_id)
            ...
            '    End If
            'End If
            '"    WHERE ref_jbooks_id IS NOT NULL AND b.status <> 'C' "  & _
    ...
    Private Sub computeGrid()
        ...
        For Each row As DataRow In dt.Rows
            credit += row.Item("debit")
            credit_based += row.Item("debit_based")
        Next
        txt_total_amount.DecimalValue = credit
        txt_total_based.DecimalValue = credit_based
    End Sub
    Private Sub begbal_ar_detail_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        ...
        If selectedRows.Count > 0 Then
            ...
            Dim myStatus = selectedRow.Cells("Status").Value.ToString
            If myStatus <> "" Then
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        'Journal ID
        '1  IB Invoicing
        '2	CR	Official Receipt
        '3	AP	Accounts Payable
        '4	CV	Check Voucher
        '5	JB	Journal Voucher
        '6	PC	Petty Cash Voucher
        '7	DC	Debit / Credit Memo
        '8	BEG	Beginning Balance
        ...
        Dim dic_ar As New Dictionary(Of String, String)

        dic_ar.Add("jbooks_ar_id", txt_jbooks_id.Value)
        dic_ar.Add("jbooks_id", txt_jbooks_id.Value)
        dic_ar.Add("terms_id", txt_terms_id.Value)
        dic_ar.Add("nodays", txt_nodays.DecimalValue)
        dic_ar.Add("dr_no", txt_dr_no.Text)
        dic_ar.Add("po_no", txt_po_no.Text)
        ...
        If trans_id = "" Then
            'Dim SQL As String = genInsert("trans_header", dic_head) & ";SELECT LAST_INSERT_ID() AS id;"
            ...
            'Dim id As DataTable = Query(Sql)
            trans_id = id.Rows(0).Item("return_id").ToString
            ...
            Dim _id As DataTable = Query(ATGenerateQuery("Insert", dbTable, dic, Environment.MachineName, Environment.UserName, main.log_id, txt_jbooks_id.Value, "jbooks_id"))
            ...
            dic_ar.Item("jbooks_id") = _id.Rows(0).Item("return_id")
            'Query(genInsert(dbTable & "_ar", dic_ar))
            Query(ATGenerateQuery("Insert", dbTable & "_ar", dic_ar, Environment.MachineName, Environment.UserName, main.log_id, txt_jbooks_id.Value, "jbooks_id"))
        ...
        ElseIf transmode = "insert" Then
            ...
            Else
                ...
                dic_ar.Item("jbooks_id") = id.Rows(0).Item("return_id")
        ...
        ElseIf transmode = "update" Then
            ...
            Else
                ...
                Dim tmp_sql = ATGenerateQuery("Update", dbTable, dic, Environment.MachineName, Environment.UserName, main.log_id, txt_jbooks_id.Value, "jbooks_id")
                Query(tmp_sql)
                ...
                If tmp.Rows.Count > 0 Then
                    ...
                    'Query(genUpdate(dbTable & "_ref", dic_ref))
                    tmp_sql = ATGenerateQuery("Update", dbTable & "_ref", dic_ref, Environment.MachineName, Environment.UserName, main.log_id, txt_jbooks_id.Value, "jbooks_id")
                    Query(tmp_sql)
                Else
                    'Query(genInsert(dbTable & "_ref", dic_ref))
                    Query(ATGenerateQuery("Insert", dbTable & "_ref", dic_ref, Environment.MachineName, Environment.UserName, main.log_id, txt_jbooks_id.Value, "jbooks_id"))
                End If
                tmp = Query(String.Format("SELECT jbooks_id FROM {0}_ar WHERE jbooks_id = {1} ", dbTable, dic_ref("jbooks_id")))
                If tmp.Rows.Count > 0 Then
                    dic_ar.Remove("jbooks_ar_id")
                    'Query(genUpdate(dbTable & "_ar", dic_ar))
                    Query(ATGenerateQuery("Update", dbTable & "_ar", dic_ar, Environment.MachineName, Environment.UserName, main.log_id, txt_jbooks_id.Value, "jbooks_id"))
                Else
                    'Query(genInsert(dbTable & "_ar", dic_ar))
                    Query(ATGenerateQuery("Insert", dbTable & "_ar", dic_ar, Environment.MachineName, Environment.UserName, main.log_id, txt_jbooks_id.Value, "jbooks_id"))
                End If
    ...
    Private Sub txt_genref_Leave(sender As Object, e As EventArgs) Handles txt_general_id.Leave
        Dim dt As DataTable = FsQuery("SELECT * FROM lib_erp_general ORDER BY FIELD(category_id,1,2),general_code ASC ")
    ...
    Private Sub btnViewDetails_Click(sender As Object, e As EventArgs) Handles btnViewDetails.Click
        ...
        If id <> "" Then
            Try
                ...
                '"WHERE b.ref_jbooks_id = '{0}' AND a.status <> 'C'", id, _serverDBFS)
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Else
                        ...
                        Dim frmDet As New begbal_det_new With {
                            .trans_no = txt_ref_no.Text,
                            .trans_date = txt_ref_date.Value,
                            .general_name = txt_general_name.Text,
                            .line_remarks = "",
                            .amount = txt_debit.DecimalValue,
                            .si_no = "",
                            .rr_no = txt_dr_no.Text,
    ...
    Private Sub btnNotSave_Click(sender As Object, e As EventArgs) Handles btnNotSave.Click
        ...
        disable_control(Me)
    End Sub
    Private Sub txt_currency_id_Leave(sender As Object, e As EventArgs) Handles txt_currency_name.Leave
    ...
    Private Sub txt_debit_Leave(sender As Object, e As EventArgs) Handles txt_debit.Leave
        ...
        If transmode = "update" Then
            ...
            If selectedRows.Count > 0 Then
                ...
                If selectedRow.Cells("minimum").Value > txt_debit.DecimalValue Then
    ...
    Private Sub txttermsiddet_Leave(sender As Object, e As EventArgs) Handles txt_terms_id.Leave
        Dim terms As DataTable = FsQuery("select * from lib_erp_terms where terms_type = 2 or terms_type = 3")
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        _printoption.cbo_sort.Items.Add("BILL NO.")
        ...
        Dim dt As DataTable = Query(sql)
        If dt.Rows.Count > 0 Then
            Dim sql_account As String = String.Format("SELECT a.jbooks_id,a.trans_id,a.project_id,a.department_id,a.allocation_id
                ,a.account_id,a.currency_id,a.exchange_rate,a.based_rate,a.debit,a.credit,a.debit_based,a.credit_based ,a.general_id,a.ref_jbooks_id,a.line_remarks
                ,b.ref_no,b.ref_date,d.general_name,d.general_code ,IF(a.credit > e.debit,'Partially Paid',IF(a.credit = e.debit,'Fully Paid','')) as status ,IF(a.credit > e.debit,e.debit,IF(a.credit = e.debit,a.credit,0)) AS minimum 
                ,f.terms_id,IFNULL(f.nodays,0) AS nodays,f.si_no,f.rr_no,f.po_no 
                ,g.terms_code,g.terms_name 
                ,h.currency_code,h.currency_name ,c.account_code,c.account_name ,a.project_unit_no_id AS unit_id,i.project_unit_no_codes AS unit_code,i.project_unit_no_names AS unit_name
                FROM trans_jbooks a
                LEFT JOIN trans_jbooks_ref b ON b.jbooks_id = a.jbooks_id
                LEFT JOIN {0}.lib_erp_chart c ON c.account_id = a.account_id
                LEFT JOIN {0}.lib_erp_general d ON d.general_id = a.general_id
                LEFT JOIN (
	                SELECT a.ref_jbooks_id AS jbooks_id,SUM(a.debit) AS debit,SUM(a.credit) AS credit
	                FROM trans_jbooks a
	                LEFT JOIN trans_header b ON b.trans_id = a.trans_id
	                WHERE ref_jbooks_id IS NOT NULL AND (b.status <> 'C' OR b.status IS NULL) GROUP BY a.ref_jbooks_id
                ) e ON e.jbooks_id = a.jbooks_id
                LEFT JOIN erp_accounting_ngg.trans_jbooks_ap f ON f.jbooks_id = a.jbooks_id
                LEFT JOIN {0}.lib_erp_terms g ON g.terms_id = f.terms_id
                LEFT JOIN {0}.lib_erp_currency h ON h.currency_id = a.currency_id
                LEFT JOIN {0}.lib_erp_project_unit_no i ON i.project_unit_no_id = a.project_unit_no_id
                LEFT JOIN {0}.lib_erp_book_unit_no j ON j.book_unit_no_id = a.book_unit_no_id
                WHERE c.validation = 2 AND a.trans_id = {1} AND a.account_id = {2}  AND a.project_id = {3} AND a.department_id = {4}", _serverDBFS, trans_id, account_id, project_id, department_id)
