...
Public Class begbal_fa_detail
    Dim dbTable As String = "trans_fixed_asset"
    ...
    Friend Sub initializeDecimalPlaces(ByVal strNumberFormat As String)
        ...
        txt_salvage_value.NumberFormat = strNumberFormat
        txt_salvage_value_based.NumberFormat = strNumberFormat
    ...
    Private Sub init_load()
        ...
        If jbooks_id <> "0" And jbooks_id <> "" Then
            Dim sql = String.Format("SELECT a.fixed_asset_id,a.jbooks_id,a.currency_id,a.exchange_rate,a.based_rate,a.description " &
                ",a.asset_code,a.historical_rate,a.quantity,a.date_acquired,a.amount,a.amount_based,a.salvage_value " &
                ",a.salvage_value_based,a.useful_life,a.date_start,a.debit,a.credit,a.debit_based,a.credit_based,a.ref_fixed_asset_id " &
                ",a.net_book_value,a.net_book_value_based,b.currency_name,c.fixed_asset_ref_id,c.ref_no " &
                ...
                "LEFT JOIN trans_fixed_asset_ref c ON c.fixed_asset_id = a.fixed_asset_id " &
                "LEFT JOIN (" &
                    "SELECT c.ref_fixed_asset_id AS fixed_asset_id,SUM(c.debit) AS debit,SUM(c.credit) AS credit " &
                    ",SUM(c.debit_based) AS debit_based,SUM(c.credit_based) AS credit_based " &
                    "FROM trans_header a " &
                    "LEFT JOIN trans_jbooks b ON b.trans_id = a.trans_id " &
                    "LEFT JOIN trans_fixed_asset c ON c.jbooks_id = b.jbooks_id " &
                    "WHERE c.ref_fixed_asset_id IS NOT NULL AND (a.status <> 'C' OR a.status IS NULL) " &
                    "GROUP BY c.ref_fixed_asset_id " &
                ") d ON d.fixed_asset_id = a.fixed_asset_id " &
        ...
        loadEnd = True
    End Sub
    Private Sub begbal_fa_detail_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub dgMain_SelectionChanged(sender As Object, e As EventArgs) Handles dgMain.SelectionChanged
        ...
        bind_control(dgMain, "fixed_asset_id", dt, Me)
    End Sub
    Private Sub begbal_fa_detail_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Dim pk As String = ""
        Dim fixed_asset_id = txt_fixed_asset_id.Value
        ...
        Dim salvage_value = txt_salvage_value.DecimalValue
        Dim salvage_value_based = salvage_value * (exchange_rate / based_rate)
        ...
        Dim debit = amount
        Dim debit_based = amount_based
        'trans_fixed_asset
        dr.Item("fixed_asset_id") = fixed_asset_id
        ...
        dr.Item("salvage_value") = salvage_value
        dr.Item("salvage_value_based") = salvage_value_based
        ...
        Else
            If transmode = "insert" Then
                If DataExist(dgMain, {"fixed_asset_id", txt_fixed_asset_id.Value}, {"ref_no"}, {txt_ref_no.Text}) Then
                    MsgBox("Fixed Asset already exist")
                    err = True
                Else
                    dr.Item("fixed_asset_id") = "ag-" & dt.Rows.Count
                    dt.Rows.Add(dr)
                    pk = dr.Item("fixed_asset_id")
                End If
            ElseIf transmode = "update" Then
                If DataExist(dgMain, {"fixed_asset_id", txt_fixed_asset_id.Value}, {"ref_no"}, {txt_ref_no.Text}, False) Then
                    MsgBox("Fixed Asset already exist")
                    err = True
                Else
                    Dim id = dgMain.SelectedRows(0).Cells("fixed_asset_id").Value.ToString
                    Dim row As DataRow = dt.Select(String.Format("fixed_asset_id = '{0}'", id))(0)
                    'trans_fixed_asset
                    ...
                    row.Item("salvage_value") = salvage_value
                    row.Item("salvage_value_based") = salvage_value_based
            ...
            If Not err Then
                ...
                For Each row As DataGridViewRow In dgMain.Rows
                    If row.Cells("fixed_asset_id").Value.ToString = pk Then
                        row.Selected = True
                    End If
                Next
            End If
    ...
    Private Sub computeNetBookValue()
        ...
        If loadEnd Then
            ...
            Dim salvage_value As Decimal = txt_salvage_value.DecimalValue
            ...
            Dim Monthly_Depreciation = If(useful_life = 0, 0, (amount - salvage_value) / useful_life)
            ...
            txt_salvage_value_based.DecimalValue = salvage_value * (exchange_rate / based_rate)
        ...
        End If
    End Sub
    Private Sub ComputeResult(sender As Object, e As EventArgs) Handles txt_useful_life.Leave, txt_salvage_value.Leave, txt_amount.Leave, txt_date_start.ValueChanged, txt_exchange_rate.Leave, txt_based_rate.Leave
        computeNetBookValue()
    ...
    Private Sub txt_date_acquired_ValueChanged(sender As Object, e As EventArgs) Handles txt_date_acquired.ValueChanged
        ...
        txt_date_start.Value = txt_date_acquired.Value
    End Sub
    ...
    Private Sub btnRemove_Click(sender As Object, e As EventArgs) Handles btnRemove.Click
        Dim selectedIDS = getCheckedGrid("fixed_asset_id", "ref_no", "sel", dgMain, , True, , True)
        If selectedIDS.Length > 0 Then
            For Each id As String In selectedIDS
                Dim selectedRows = ds.Tables(dbTable).Select(String.Format("fixed_asset_id = '{0}'", id))
                If selectedRows.Count > 0 Then
                    Dim selectedRow = selectedRows(0)
    ...
    Private Sub btnViewDetails_Click(sender As Object, e As EventArgs) Handles btnViewDetails.Click
        Dim id As String = txt_fixed_asset_id.Value.ToString
        If id <> "" Then
            Try
                ...
                Dim sql = String.Format("SELECT a.trans_no AS ref_no,a.trans_date,d.book_code,e.account_code,e.account_name,b.debit,b.credit " & _
                    ...
                    "LEFT JOIN trans_fixed_asset c ON c.jbooks_id = b.jbooks_id " & _
                    "LEFT JOIN {1}.lib_erp_book d ON d.book_id = a.book_id " & _
                    "LEFT JOIN {1}.lib_erp_chart e ON e.account_id = b.account_id " & _
                    "WHERE c.ref_fixed_asset_id = '{0}' AND (a.status <> 'C' OR a.status IS NULL) ", id, _serverDBFS)
    ...
    Private Sub txt_ref_no_Leave(sender As Object, e As EventArgs) Handles txt_ref_no.Leave
        ...
        Else
            If transmode = "insert" Then
                Dim sql As String = String.Format("SELECT fixed_asset_id FROM trans_fixed_asset_ref WHERE ref_no = '{0}'", ref_no)
                Dim dt As DataTable = Query(sql)
                If dt.Rows.Count > 0 Then
                ...
                End If
            Else
                Dim sql As String = String.Format("SELECT fixed_asset_id FROM trans_fixed_asset_ref WHERE ref_no = '{0}' AND fixed_asset_id <> '{1}'", ref_no, txt_fixed_asset_id.Text)
                Dim dt As DataTable = Query(sql)
                If dt.Rows.Count > 0 Then
                    If txt_ref_no.Enabled Then
                        MsgBox("Reference Number Already Exist.")
    ...
    Private Sub begbal_fa_detail_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        ...
        End If
        If ds.Tables("trans_fixed_asset").Rows.Count = 0 Then
            ds.Tables("trans_fixed_asset").Clear()
            Dim dt_iv As DataTable = dgMain.DataSource
            For Each dr As DataRow In dt_iv.Rows
                ds.Tables("trans_fixed_asset").ImportRow(dr)
            Next
        End If
        If ds.Tables("trans_fixed_asset").Rows.Count > 0 Then
            If My.Settings._enableUnitNo Then
                For Each dr As DataRow In ds.Tables("trans_fixed_asset").Rows
                    If Not lstUnitID.ContainsKey(dr("unit_id")) Then
                        ...
                        If drs.Length > 0 Then
                            ...
                            If jbooks_id.Contains("tmp_") Or jbooks_id = "" Then
                                jbooks_debit = ds.Tables("trans_fixed_asset").Compute("SUM(debit)", String.Format("unit_id = '{0}' AND deleted = False", dr("unit_id")))
                                jbooks_debit_based = ds.Tables("trans_fixed_asset").Compute("SUM(debit_based)", String.Format("unit_id = '{0}' AND deleted = False", dr("unit_id")))
                ...
                Next
                For Each dr As DataRow In ds.Tables("trans_fixed_asset").Rows
                    Dim fixed_asset_ref_id = dr("fixed_asset_ref_id")
                    Dim fixed_asset_id = dr("fixed_asset_id")
                    ...
                    Dim salvage_value = dr("salvage_value")
                    Dim salvage_value_based = dr("salvage_value_based")
                    Dim net_book_value = dr("net_book_value")
                    Dim net_book_value_based = dr("net_book_value_based")
                    Dim useful_life = dr("usefule_life")
                    ...
                    End If
                    Dim dic As New Dictionary(Of String, String)
                    dic.Add("fixed_asset_id", fixed_asset_id)
                    'dic.Add("ref_no", ref_no)
                    dic.Add("jbooks_id", jbooks_id)
                    ...
                    dic.Add("salvage_value", salvage_value)
                    dic.Add("salvage_value_based", salvage_value_based)
                    ...
                    If fixed_asset_id.ToString.Contains("ag-") Or fixed_asset_id.ToString.Contains("tmp_") Or fixed_asset_id = "" Then
                        If Not deleted Then
                            Dim sql As String = ATGenerateQuery("Insert", "trans_fixed_asset", dic, Environment.MachineName, Environment.UserName, main.log_id, "|", "fixed_asset_id")
                            Dim return_id = Query(sql)
                            fixed_asset_id = return_id.Rows(0)("return_id")
                            Dim dicDet As New Dictionary(Of String, String)
                            dicDet.Add("fixed_asset_ref_id", "")
                            dicDet.Add("fixed_asset_id", fixed_asset_id)
                            dicDet.Add("ref_no", ref_no)
                            Dim sql_ref As String = ATGenerateQuery("Insert", "trans_fixed_asset_ref", dicDet, Environment.MachineName, Environment.UserName, main.log_id, "|", "fixed_asset_ref_id")
                            Query(sql_ref)
                        End If
                    Else
                        If deleted Then
                            Dim sql As String = ATGenerateQuery("DELETE", "trans_fixed_asset", Nothing, Environment.MachineName, Environment.UserName, main.log_id, fixed_asset_id & "|", "fixed_asset_id")
                            Query(sql)
                        Else
                            Dim sql As String = ATGenerateQuery("UPDATE", "trans_fixed_asset", dic, Environment.MachineName, Environment.UserName, main.log_id, fixed_asset_id & "|", "fixed_asset_id")
                            Query(sql)
                            Dim return_id = Query(sql)
                            fixed_asset_id = return_id.Rows(0)("return_id")
                            Dim dicDet As New Dictionary(Of String, String)
                            dicDet.Add("fixed_asset_ref_id", fixed_asset_ref_id)
                            dicDet.Add("fixed_asset_id", fixed_asset_id)
                            dicDet.Add("ref_no", ref_no)
                            Dim sql_ref As String = ATGenerateQuery("Update", "trans_fixed_asset_ref", dicDet, Environment.MachineName, Environment.UserName, main.log_id, fixed_asset_ref_id & "|", "fixed_asset_ref_id")
                            Query(sql_ref)
                        End If
                    End If
                Next
            Else
                ...
                If drs.Length > 0 Then
                    jbooks_id = drs(0)("jbooks_id")
                    If jbooks_id.Contains("tmp_") Or jbooks_id = "" Then
                        jbooks_debit = ds.Tables("trans_fixed_asset").Compute("SUM(debit)", String.Format("deleted = False"))
                        jbooks_debit_based = ds.Tables("trans_fixed_asset").Compute("SUM(debit_based)", String.Format("deleted = False"))
                        If jbooks_debit <> 0 Or jbooks_debit_based <> 0 Then
                            Dim dic As New Dictionary(Of String, String)
                            dic.Add("jbooks_id", "")
                            dic.Add("trans_id", trans_id)
