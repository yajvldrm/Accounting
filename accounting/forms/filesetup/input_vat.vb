...
Public Class input_vat
    Dim dbTable As String = "lib_acc_input_vat"
    Dim ds As New FileSetupSet
    Private transmode As String = ""
    Private module_code As String = "ACC00816"
    ...
    Private Sub init_load()
        ...
        Dim sql = "SELECT * FROM lib_acc_input_vat ORDER BY effectivity_date DESC"
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Else
            If transmode = "insert" Then
                If DataExist(dgMain, {"input_vat_id", txt_input_vat_id.Value}, {"effectivity_date"}, {txt_effectivity_date.Value & " 12:00:00 AM"}) Then
                    MsgBox("Input VAT already exist")
                    err = True
                Else
                    'Dim id As DataTable = FsQuery(genInsert(dbTable, dic) & ";SELECT LAST_INSERT_ID() AS id;")
                    dic.Remove("sel")
                    Dim id As DataTable = FsQuery(ATGenerateQuery("Insert", dbTable, dic, Environment.MachineName, Environment.UserName, main.log_id, txt_input_vat_id.Value, "input_vat_id"))
                    pk = id.Rows(0).Item("return_id")
                    addsave = True
                End If
            ElseIf transmode = "update" Then
                If DataExist(dgMain, {"input_vat_id", txt_input_vat_id.Value}, {"effectivity_date"}, {txt_effectivity_date.Value & " 12:00:00 AM"}, False) Then
                    MsgBox("Input VAT already exist")
                    err = True
                Else
                    dic.Remove("sel")
                    'FsQuery(genUpdate(dbTable, dic))
                    FsQuery(ATGenerateQuery("Update", dbTable, dic, Environment.MachineName, Environment.UserName, main.log_id, txt_input_vat_id.Value, "input_vat_id"))
                    pk = txt_input_vat_id.Value
            ...
            If Not err Then
                ...
                Next
                If addsave Then
                    saveAddAnother(btnInsert, "Add New Input Vat?")
                End If
            End If
        End If
    End Sub
    ...
    Private Sub btnRemove_Click(sender As Object, e As EventArgs) Handles btnRemove.Click
        ...
        Dim tbl_delete As String = "lib_acc_input_vat"
        Dim pk_delete As String = "input_vat_id"
        Dim msg As String = ""
        ...
        If arr_id.Length > 0 Then
            ...
            Next
            'Query(str)
            str = ATGenerateQuery("Remove", dbTable, Nothing, Environment.MachineName, Environment.UserName, main.log_id, ids, "input_vat_id")
            Query(str)
            init_load()
        End If
    End Sub
