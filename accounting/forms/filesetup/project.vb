...
Public Class project
    Dim dbTable As String = "lib_erp_project"
    Dim ds As New FileSetupSet
    Private transmode As String = ""
    Private module_code As String = "ACC00813"
    ...
    Private Sub init_load()
        ...
        Dim sql = "SELECT * FROM lib_erp_project a ORDER BY a.project_code,a.project_name ASC "
    ...
    Private Sub dgMain_SelectionChanged(sender As Object, e As EventArgs) Handles dgMain.SelectionChanged
        ...
        bind_control(dgMain, "project_id", dt, Me)
    End Sub
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Else
            If transmode = "insert" Then
                If DataExist(dgMain, {"project_id", txt_project_id.Value}, {"project_code", "project_name"}, {txt_project_code.Text, txt_project_name.Text}) Then
                    MsgBox("Project already exist")
                    err = True
                Else
                    If dic.ContainsKey("sel") Then
                        dic.Remove("sel")
                    End If
                    Dim id As DataTable = FsQuery(ATGenerateQuery("Insert", dbTable, dic, Environment.MachineName, Environment.UserName, main.log_id, txt_project_id.Value, "project_id"))
            ...
            ElseIf transmode = "update" Then
                If DataExist(dgMain, {"project_id", txt_project_id.Value}, {"project_code", "project_name"}, {txt_project_code.Text, txt_project_name.Text}, False) Then
                    MsgBox("Project already exist")
                    err = True
                Else
                    dic.Remove("sel")
                    FsQuery(ATGenerateQuery("Update", dbTable, dic, Environment.MachineName, Environment.UserName, main.log_id, txt_project_id.Value, "project_id"))
                    'FsQuery(genUpdate(dbTable, dic))
                    pk = txt_project_id.Value
            ...
            If Not err Then
                ...
                For Each row As DataGridViewRow In dgMain.Rows
                    If row.Cells("project_id").Value.ToString = pk Then
                        row.Cells(0).Selected = True
                    End If
                Next
                If addsave Then
                    saveAddAnother(btnInsert, "Add New Project?")
                End If
            End If
        End If
    End Sub
    ...
    Private Sub btnRemove_Click(sender As Object, e As EventArgs) Handles btnRemove.Click
        ...
        Dim tbl_delete As String = "lib_erp_project"
        Dim pk_delete As String = "project_id"
        Dim msg As String = ""
        ...
        ElseIf arr_id.Length > 0 Then
            ...
            Next
            'Query(str)
            FsQuery(ATGenerateQuery("Remove", dbTable, Nothing, Environment.MachineName, Environment.UserName, main.log_id, ids, "project_id"))
            init_load()
        End If
    End Sub
    ...
    Private Sub txtsearch_TextChange(sender As Object, e As EventArgs) Handles txtsearch.TextChange
        ...
        If txtsearch.TextSource <> "" Then
            ...
            If selectedRows.Length > 0 Then
                ...
                'End If
                'Dim selectedID As String = selectedRows(currentSearchIndex).Item("project_id").ToString
                'dgMain.ClearSelection()
                'For Each row As DataGridViewRow In dgMain.Rows
                '    If row.Cells("project_id").Value.ToString = selectedID Then
                '        row.Selected = True
